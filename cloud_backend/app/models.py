...
class OrgRoleTemplateRecord(BaseModel):
    ...
    sortOrder: int = 0
    active: bool = True
    # 用户甲 5/24 V2.1 lab: 持岗人为机器人同事时填 bot_member.id; 旧数据无该字段, 按 null 处理.
...
class TaskCollaboratorRecord(BaseModel):
    ...
    # for archived employees like "archived+xxx@example.invalid" — RFC reserves ".invalid" TLD so
