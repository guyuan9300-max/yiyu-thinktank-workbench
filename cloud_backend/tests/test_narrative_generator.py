...
def db(tmp_path: Path) -> Database:
    ...
    database.execute(
        ...
        (CLIENT_ID, ORG_ID, CLIENT_NAME, "A组织", "client", now, now),
    )
    database.execute(
    ...
    database.execute(
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (
            "el_brand", ORG_ID, "A组织品牌改造", "project", "active", "discovery",
            "为A组织做品牌升级 + 视觉系统重构",
            "客户希望在年底前完成新品牌发布", "等老师乙反馈时间规划",
    ...
    database.execute(
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (
            "tsk_1", ORG_ID, "A组织|品牌改造|准备 visual story 提纲", "",
...
def test_collector_picks_up_event_lines_and_activities(db: Database) -> None:
    ...
    assert el.name == "A组织品牌改造"
    assert "负责人甲" not in el.summary  # summary 不应被乱拼
    assert "品牌升级" in el.summary
    assert len(ctx.activities) == 1
    assert ctx.activities[0].title == "A组织秘书长见面纪要"
    assert ctx.activities[0].event_line_name == "A组织品牌改造"
    assert len(ctx.tasks) == 1
    assert ctx.tasks[0].title.startswith("A组织|品牌改造")
...
def test_collector_pending_vs_applied_clarifications(db: Database) -> None:
    svc.add_clarification(
        db, ORG_ID, CLIENT_ID,
        NarrativeClarificationCreatePayload(dimension="people", answer="负责人甲是秘书长"),
...
def test_generator_falls_back_to_stub_when_no_api_key(db: Database, monkeypatch) -> None:
    ...
    svc.add_clarification(
        db, ORG_ID, CLIENT_ID,
        NarrativeClarificationCreatePayload(dimension="people", answer="负责人甲是A组织秘书长"),
        answered_by_user_id=USER_ID,
        answered_by_display_name=USER_NAME,
    )
...
def test_generator_uses_llm_response_when_ok(db: Database, monkeypatch) -> None:
    ...
    fake_llm = {
        "essence": {
            "narrative": "A组织是儿童社会情感学习领域的公益组织, 项目类型为品牌升级综合方案",
            "confidence": "high",
            "confidenceReason": "基于 event_line summary + activity 见面纪要",
            "references": [
                {"sourceType": "event_line", "sourceId": "el_brand", "label": "A组织品牌改造主线", "confidence": "high"},
        ...
        "people": {
            "narrative": "负责人甲秘书长 — 决策者; 徐总监 — 经办; 老师乙 (示例团队方) — 主导",
            "confidence": "medium",
        ...
        "timeline": {
            "narrative": "4/12 老师乙与负责人甲见面后确认方向...",
            "confidence": "medium",
            "references": [
        ...
        "next_steps": {
            "narrative": "示例团队 → 5/15 前出 visual story 提纲; 负责人甲确认方向",
            "confidence": "medium",
...
def test_build_user_prompt_contains_real_facts(db: Database) -> None:
    ...
    assert "A组织品牌改造" in prompt
    assert "负责人甲" in prompt   # 来自 activity summary
