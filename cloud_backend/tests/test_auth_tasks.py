...
def auth_headers(client: TestClient, email: str = "admin@example.org", password: str = "Admin123!"):
...
def seed_registration_departments(app) -> None:
    ...
    timestamp = now_iso()
    db.execute("UPDATE organizations SET name = ?, updated_at = ? WHERE id = ?", ("示例工作台", timestamp, DEFAULT_ORG_ID))
    # bcrypt hash 比较贵, 共用一个 hash; fixture 用户密码都是 Simulate123! (admin 用 Admin123!)。
    sim_hash = hash_password("Simulate123!")
    admin_hash = hash_password("Admin123!")
    fixture_users = [
        ("user_admin_demo", "admin-demo@example.org", "管理员甲", "admin", admin_hash, None, None, 1),
        ("user_qinghua", "member-a@example.org", "庆华", "employee", sim_hash, "dept_customer_service", "客户服务部", 1),
        ("user_jianing", "member-b@example.org", "佳乐", "employee", sim_hash, "dept_customer_service", "客户服务部", 0),
        ("user_yishuo", "member-c@example.org", "一朔", "employee", sim_hash, "dept_info_data", "信息数据部", 0),
    ]
    for user_id, email, full_name, role, pwd_hash, department_id, department_name, is_department_lead in fixture_users:
        db.execute(
            """
            INSERT INTO employee_accounts(
                id, organization_id, email, full_name, password_hash, primary_role, account_status,
                membership_status, approved_at, approved_by, recent_mentions_json,
                department_id, department_name, is_department_lead, created_at, updated_at
    ...
    for user_id, _, _, role, _, _, _, _ in fixture_users:
        db.execute(
            """
            INSERT OR IGNORE INTO employee_role_bindings(id, user_id, role, created_at)
            VALUES(?, ?, ?, ?)
            """,
            (f"role_binding_{user_id}", user_id, role, timestamp),
        )
    for user_id, _, _, _, _, department_id, department_name, _ in fixture_users:
        if department_id:
            db.execute(
                """
                UPDATE employee_accounts
                 ...
                 WHERE id = ?
                """,
                (department_id, department_name, timestamp, user_id),
            )
    for user_id, email, *_rest in fixture_users:
        # Keep older rows created by email-only tests aligned with the canonical fixture ids.
        db.execute(
            "UPDATE employee_accounts SET id = ? WHERE email = ? AND id != ?",
            (user_id, email.lower(), user_id),
        )
    ...
    for role_id, department_id, name, sort_order, level, is_manager, task_edit_scope, can_approve, can_reassign, can_change_deadline in [
        ("role_cs_lead", "dept_customer_service", "客户服务负责人", 0, "department_lead", 1, "department", 1, 1, 1),
        ("role_cs_member", "dept_customer_service", "客户推进", 1, "employee", 0, "self", 0, 0, 0),
        ("role_info_lead", "dept_info_data", "信息数据负责人", 0, "department_lead", 1, "department", 1, 1, 1),
        ("role_info_member", "dept_info_data", "信息分析", 1, "employee", 0, "self", 0, 0, 0),
    ]:
        db.execute(
            """
    ...
    db.execute(
        ...
        ON CONFLICT(id) DO UPDATE SET
            ...
            content_editable_by = excluded.content_editable_by,
            ...
            require_collab_confirmation = excluded.require_collab_confirmation,
    ...
    for user_id in ["user_admin_demo", "user_qinghua", "user_jianing", "user_yishuo"]:
        cloud_main._sync_employee_org_binding_from_account(app.state.app_state, DEFAULT_ORG_ID, user_id, fill_missing_only=False)
    db.execute(
        """
        ...
        """,
        ("line_qinghua_jianing", DEFAULT_ORG_ID, "user_qinghua", "user_jianing", timestamp),
    )
...
def test_register_approve_login_and_collaboration_flow():
    ...
    invited_login = client.post("/api/v1/auth/login", json={"email": "new-user@example.org", "password": "Password123!"})
    assert invited_login.status_code == 200, invited_login.text
    assert invited_login.json()["user"]["membershipStatus"] == "approved"
    invited_headers = {"Authorization": f"Bearer {invited_login.json()['accessToken']}"}
    allowed_tasks = client.get("/api/v1/tasks", headers=invited_headers)
    assert allowed_tasks.status_code == 200, allowed_tasks.text

    admin_headers = auth_headers(client)
    employees = client.get("/api/v1/admin/employees", headers=admin_headers)
    pending_user = next(item for item in employees.json() if item["email"] == "new-user@example.org")
    ...
    assert pending_user["managerName"] == "管理员甲"
    ...
    assert approved_binding["managerUserId"] == "user_admin_demo"
    ...
    assert approved_line["managerUserId"] == "user_admin_demo"
    ...
    assert patched_binding["managerUserId"] == "user_admin_demo"
    assert patched_binding["projectRoleLabels"] == ["客户成功专员"]

    user_headers = auth_headers(client, "new-user@example.org", "Password123!")
    ...
    qinghua_headers = auth_headers(client, "member-a@example.org", "Simulate123!")
    ...
    jianing_headers = auth_headers(client, "member-b@example.org", "Simulate123!")
...
def test_feishu_binding_relay_session_roundtrip():
    ...
    assert "飞书绑定失败" in callback_response.text
    assert "当前组织尚未完成飞书接入" in callback_response.text
...
def test_task_overdue_only_after_calendar_day_ends():
    ...
    admin_login = client.post("/api/v1/auth/login", json={"email": "admin@example.org", "password": "Admin123!"})
    ...
    register = client.post(
        "/api/v1/auth/register",
        json={
            "email": "overdue-check@example.org",
            "fullName": "逾期校验员",
            "password": "Password123!",
    ...
    pending_user = next(item for item in employees.json() if item["email"] == "overdue-check@example.org")
    ...
    user_login = client.post("/api/v1/auth/login", json={"email": "overdue-check@example.org", "password": "Password123!"})
...
def test_org_model_profile_roundtrip():
    ...
    assert response.status_code == 200, response.text
    payload = response.json()
    assert payload['organization']['name'] == '示例工作台'
...
def test_task_org_link_and_department_control_permissions():
    ...
    assert create_payload["orgContext"]["roleTemplateId"] == "role_cs_member"
    ...
    assert str(link_row["role_template_id"]) == "role_cs_member"
...
def test_task_plan_link_and_support_request_flow():
    ...
    yishuo_headers = auth_headers(client, "member-c@example.org", "Simulate123!")
    create = client.post(
        "/api/v1/tasks",
        json={
            "title": "管理信号模板接入周总结",
    ...
    assert recompute_payload["departmentPlanItemId"] in {None, "plan_item_signal"}
...
def test_task_review_approve_and_return_follow_org_permissions():
    ...
    register = client.post(
        "/api/v1/auth/register",
        json={
            "email": "review-worker@example.org",
            "fullName": "复核执行员",
            "password": "Password123!",
            "inviteCode": customer_service_invite,
            "departmentId": "dept_customer_service",
    ...
    employees = client.get("/api/v1/admin/employees", headers=admin_headers)
    assert employees.status_code == 200, employees.text
    pending_user = next(item for item in employees.json() if item["email"] == "review-worker@example.org")
    approve = client.post(
        f"/api/v1/admin/employees/{pending_user['id']}/approve",
        json={"role": "employee"},
        headers=admin_headers,
    ...
    assert approve.status_code == 200, approve.text
    worker_headers = auth_headers(client, "review-worker@example.org", "Password123!")
