...
def test_mapped_sender_can_query_today_tasks_and_logs_result(tmp_path, monkeypatch):
    ...
    headers, user = auth_headers(client, "admin@example.org", "Admin123!")
...
def test_model_parse_can_filter_tasks_by_collaboration_partner(tmp_path, monkeypatch):
    client = make_client(tmp_path, monkeypatch)
    headers, admin_user = auth_headers(client, "admin@example.org", "Admin123!")
    ensure_member(client, user_id="user_qinghua", organization_id=admin_user["organizationId"], full_name="庆华", email="member-a@example.org")
    ensure_member(client, user_id="user_jianing", organization_id=admin_user["organizationId"], full_name="佳宁", email="member-b@example.org")
    _, qinghua_user = auth_headers(client, "member-a@example.org", "Simulate123!")
    _, jianing_user = auth_headers(client, "member-b@example.org", "Simulate123!")
    organization_id = save_org_feishu_integration(client, headers, monkeypatch)
    disable_outbound_feishu(monkeypatch, client)
