...
def test_secure_bootstrap_defaults_do_not_accept_source_credentials(tmp_path: Path, monkeypatch):
    ...
    default_login = client.post('/api/v1/auth/login', json={'email': 'admin@example.org', 'password': 'Admin123!'})
    ...
    bootstrap_login = client.post('/api/v1/auth/login', json={'email': 'admin@example.org', 'password': bootstrap_password})
...
def test_seed_password_from_env_refreshes_existing_admin_login(tmp_path: Path, monkeypatch):
    ...
    first_login = first_client.post('/api/v1/auth/login', json={'email': 'admin@example.org', 'password': 'Admin123!'})
    ...
    old_login = second_client.post('/api/v1/auth/login', json={'email': 'admin@example.org', 'password': 'Admin123!'})
    assert old_login.status_code == 401, old_login.text

    new_login = second_client.post('/api/v1/auth/login', json={'email': 'admin@example.org', 'password': 'Admin456!'})
