...
def db_seeded(db: Database) -> Database:
    ...
    db.conn.executemany(
        """INSERT INTO commitments(id, client_id, committer, recipient, commitment_type,
           ...
           VALUES(?, 'c1', ?, ?, 'delivery', ?, ?, ?, ?, ?)""",
        [
            ("cm_p1", "管理员甲", "客户", "交方案 v1", "2026-06-01", "pending", now, now),
            ("cm_p2", "乐乐", "客户", "回信件",   "2026-05-25", "pending", now, now),
            ("cm_f1", "管理员甲", "客户", "送资料",  "2026-05-10", "fulfilled", now, now),
            ("cm_c1", "管理员甲", "客户", "已撤回",  "2026-05-30", "cancelled", now, now),
        ],
    )
    db.conn.executemany(
        ...
        [
            ...
            ("gt_2", "青年支持计划", "青年支持计划", "员工心理", "[]", now, now),
        ],
    )
    db.conn.commit()
    return db
...
def test_glossary_list_paginated_returns_total(db_seeded: Database):
    ...
    assert total == 2
    assert len(terms) == 2
    assert {t.term for t in terms} == {"战略陪伴", "青年支持计划"}
