...
def test_create_get_basic(db: sqlite3.Connection) -> None:
    ...
    created = repo.create(
        ClientCreatePayload(name="A组织", alias="A组织", intro="心理健康公益")
    )
    assert created.name == "A组织"
    assert created.alias == "A组织"
...
def test_get_by_alias(db: sqlite3.Connection) -> None:
    repo = ClientRepository(db)
    repo.create(ClientCreatePayload(name="A组织", alias="A组织"))
    found = repo.get_by_alias("A组织")
    assert found is not None
    assert found.name == "A组织"
...
def test_freeze_unfreeze(db: sqlite3.Connection) -> None:
    repo = ClientRepository(db)
    created = repo.create(ClientCreatePayload(name="A组织"))
