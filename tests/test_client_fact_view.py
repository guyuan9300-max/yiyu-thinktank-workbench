...
- 金标准: 真实A组织 db 跑出 NORTH_STAR 目标 B 的第一个里程碑
"""
...
def db() -> sqlite3.Connection:
    ...
    # client_stage_audit 表 schema 到 backend/app/db.py, 但本 fixture 没同步加。
    # ClientRepository.archive() / freeze() 等会写 audit log, 找不到表就抛
    # OperationalError, 导致 4 个测试 pre-existing failure
    # (test_get_fact_bundle_archived_returns_none 等)。
    # 这里加上 schema 跟 db.py 对齐。
    conn.execute(
        """
...
def japan_client_id(db: sqlite3.Connection) -> str:
    ...
    created = repo.create(
        ClientCreatePayload(name="A组织", alias="A组织", stage="active")
    )
    return created.id
...
def test_list_event_lines_returns_only_client_owned(
    ...
    db.execute(
        ...
        """,
        (
            "eline_1", "A组织战略陪伴", "project_line", "paused", "本周推进",
            "Q1 三个项目复盘中", "教师项目设计需补完", "", "", "",
            83, "管理员甲", japan_client_id, "A组织",
    ...
    assert event_lines[0].name == "A组织战略陪伴"
    assert event_lines[0].evidence_count == 83
    assert event_lines[0].owner_name == "管理员甲"
...
def test_list_commitments(
    ...
    db.execute(
        ...
        """,
        (
            "cm1", japan_client_id, "管理员甲", "负责人甲", "delivery",
    ...
    assert commitments[0].committer == "管理员甲"
    assert commitments[0].recipient == "负责人甲"
...
def test_list_dna_documents_lite(
    ...
    db.execute(
        ...
        (
            ...
            "A" * 10000, "normalized text", "A组织简介",
            "intro.md", "hash1", "manual",
            "2026-03-17", "user_admin_demo",
        ),
    )
...
def test_atomic_fact_has_provenance(
    ...
    db.execute(
        ...
        """,
        (
            "af1", japan_client_id, "负责人甲", "职务", "理事长", "理事长", 0.95,
...
def test_get_fact_bundle_complete(
    ...
    db.execute(
        ...
        """,
        (
            "task_1", japan_client_id, "和A组织负责人甲进行5月份第一次战略对齐会",
    ...
    db.execute(
        ...
        (
            japan_client_id, "organization_intro", "组织介绍",
            "# A组织\n2013 年成立", "", "A组织简介",
            "org.md", "hash1", "manual", "2026-03-17", "user_admin_demo",
        ),
    )
    # atomic_fact
    db.execute(
        ...
        """,
        (
            "af1", japan_client_id, "负责人甲", "职务", "理事长", "理事长",
            "2026-05-19", "2026-05-19",
        ),
    )
    ...
    assert bundle.event_lines[0].name == "A组织战略陪伴"
    assert "负责人甲" in bundle.tasks[0].title
    assert bundle.commitments[0].content == "撰写价值观调研问题"
    assert bundle.dna_documents[0].module_key == "organization_intro"
    assert bundle.atomic_facts[0].subject_text == "负责人甲"
    # provenance
...
def test_japan_client_fact_bundle_real_db() -> None:
    ...
    - client: A组织 (stage='待导入资料' — 自定义 stage, 不在 Literal 但 view 容错)
    - event_lines: 3 条 (A组织战略陪伴 + 建立重点客户周跟进节奏表 + 约见A组织负责人甲看示例团队系统)
    ...
    bundle = view.get_fact_bundle("client_demo_alpha")
    assert bundle is not None, "A组织 client_demo_alpha 应当存在"
    assert "A组织" in bundle.client.name

    # 事件线: 至少 1 条, 应包含'A组织战略陪伴'
    assert bundle.counts["event_lines"] >= 1, (
        f"应至少 1 条事件线, 实际 {bundle.counts['event_lines']}"
    )
    assert any(el.name == "A组织战略陪伴" for el in bundle.event_lines), (
        f"应该有'A组织战略陪伴'事件线, 实际: "
        f"{[e.name for e in bundle.event_lines]}"
    )

    # 任务: 默认 view 不含 done, 真实数据A组织所有任务都是 done, 所以默认拿到 0
    # 用 include_done=True 拿全部 - 这才能验证 ClientFactView 真实可用
    all_tasks = view.list_tasks("client_demo_alpha", include_done=True, limit=500)
    assert len(all_tasks) >= 5, (
        f"A组织历史上至少有 5 个任务, 实际 {len(all_tasks)}"
    )
    assert any("负责人甲" in t.title for t in all_tasks), (
        f"应该有负责人甲相关任务, 实际任务标题: {[t.title for t in all_tasks[:5]]}"
    )
...
def test_japan_5_19_meeting_data_visible() -> None:
    """NORTH_STAR §1 目标 B 关键验收: 5/19 负责人甲会议相关事实是否能从 ClientFactView 拿到
    ...
    matching_5_19 = [
        ...
        if "负责人甲" in t.title and ("5月" in t.title or "5月份" in t.title)
    ]
    assert len(matching_5_19) >= 1, (
        f"应该有 5/19 负责人甲对齐会任务, 实际匹配: {[t.title for t in matching_5_19]}"
    )
