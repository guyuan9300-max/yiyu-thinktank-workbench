...
def db_with_data(db: Database) -> Database:
    ...
    # 3 clients:client_a (active), client_b (active, related to user_admin_demo), client_c (frozen)
    db.conn.executemany(
        ...
        [
            ...
            ("client_b", "客户B", now, now, json.dumps(["user_admin_demo"]), None),
...
def test_v_user_visible_clients_join(db_with_data: Database):
    """user_admin_demo 应该能看到 client_b(被关联)+ client_a/无人关联的可由 LEFT JOIN 显示"""
    ...
    # client_b (关联了 user_admin_demo) → viewer_user_id = user_admin_demo
    pairs = [(r["id"], r["viewer_user_id"]) for r in rows]
    assert ("client_a", None) in pairs
    assert ("client_b", "user_admin_demo") in pairs
