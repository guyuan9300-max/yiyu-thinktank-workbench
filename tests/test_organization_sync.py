...
def _employees_v1() -> list[dict]:
    ...
    return [
        ...
        {"id": "emp_efdd076c31", "fullName": "乐乐", "email": "member-a@example.org",
         ...
         "updatedAt": "2026-05-19T07:11:27"},
        # pending:已申请但未审批
        {"id": "emp_pending_001", "fullName": "申请中员工", "email": "pending@example.org",
        ...
        # disabled:被禁用
        {"id": "emp_disabled_001", "fullName": "已禁用员工", "email": "disabled@example.org",
...
def test_first_sync_writes_all_4_tables(db: Database):
    ...
    _seed_local_clients(db, [
        ("client_a", "客户A", ["user_admin_demo", "emp_efdd076c31"]),
        ("client_b", "客户B", ["user_admin_demo"]),
    ])
    ...
    assert len(rows) == 1 and rows[0]["name"] == "示例工作台"
    ...
    assert "管理员甲" in full_names and "乐乐" in full_names and "成员乙" in full_names and "系统管理员" in full_names
    by_name = {r["full_name"]: r for r in rows}
    assert by_name["管理员甲"]["department_id"] == "department_gq160gdz"
    assert by_name["管理员甲"]["is_manager"] == 1
    assert by_name["管理员甲"]["is_department_lead"] == 1
    ...
    assert len(rows) == 3
    assert rows[0]["client_id"] == "client_a" and rows[0]["user_id"] == "user_admin_demo"
    assert rows[1]["client_id"] == "client_a" and rows[1]["user_id"] == "emp_efdd076c31"
    assert rows[2]["client_id"] == "client_b" and rows[2]["user_id"] == "user_admin_demo"
...
def test_second_sync_removes_deleted_user(db: Database):
    ...
    # 其他 3 人还在
    assert db.fetchone("SELECT 1 FROM mirror_users WHERE id='user_admin_demo'") is not None
...
def test_second_sync_removes_deleted_client_relation(db: Database):
    ...
    _seed_local_clients(db, [("client_a", "A", ["user_admin_demo", "emp_efdd076c31"])])
    ...
    assert len(rows) == 1 and rows[0]["user_id"] == "user_admin_demo"
...
def test_sync_writes_bypass_readonly_trigger(db: Database):
    ...
    rows = db.fetchall("SELECT synced_from_cloud_at FROM mirror_users WHERE id='user_admin_demo'")
...
def test_derive_cru_disabled_keeps_table_untouched(db: Database):
    ...
    _seed_local_clients(db, [("client_a", "A", ["user_admin_demo"])])
    sync_organization_directory(
        db, cloud_base_url="http://t", cloud_token="x",
...
@pytest.mark.integration
def test_user_admin_demo_role_data_correct(db: Database):
    """验证 binding 数据正确合并到 user(管理员甲 = CEO,task_edit_scope=department)"""
    sync_organization_directory(
        db, cloud_base_url="http://t", cloud_token="x",
        derive_cru_from_local=False,
        http_get=_make_mock_http(_profile_v1(), _employees_v1()),
    ...
    row = db.fetchone("""
        ...
        FROM mirror_users WHERE id='user_admin_demo'
    """)
