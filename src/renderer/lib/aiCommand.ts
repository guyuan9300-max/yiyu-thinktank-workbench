 ...
 *
 * 用户甲 5/24 任务书 §7-8: 智能指令解析 + 复杂任务类型分类
 *
...
export type ParsedSmartCommand = {
   ...
   * 例: "参考A组织的合同结构" → references=['A组织'], 不是 client_name.
   */
  ...
  /**
   * 用户甲 5/25 真用反馈: "我理解的任务" 不能是一坨, 要 step list,
...
/**
 * 三段式步骤 — 让用户可视化"成员甲是否理解了任务".
 *  · action:      成员甲要做的动作 (动词短语, 比如"写一份背景档案")
 *  · basis:       成员甲做这一步基于的输入 (虚构客户设定 / 参考 / 前置 step 产出)
 *  · deliverable: 交付物 + 落点 (篇幅/位置, 比如"~10000 字, 放进客户工作台")
 * 三段都允许"未识别"(空字符串), UI 显示灰字"(未识别, 成员甲可能误解)".
 */
...
// @ 机器人识别 (支持 @成员甲 / @成员甲 / @qinghua)
function extractBotHandle(text: string): string | null {
  const match = text.match(/@([一-龥a-zA-Z0-9_]+)/);
  return match ? match[1] : null;
}

// 用户甲 5/25 真用 bug: "参考A组织的合同结构" 把A组织识别成主客户.
// 真主客户是安然集团 (虚构, 但在 knownClientNames). A组织只是参考样本.
...
// 用户甲 5/25 真用 bug: extractOutputs 抓到了元话语 + 双冒号 + 重复.
// 真截图问题:
...
function extractOutputs(text: string): string[] {
  ...
  // 用户甲 5/24 反馈: 真用指令里有"写一份/拟一份/建一个任务/给我建/做一份"等.
...
// ── 执行计划数据结构 (用户甲 §9 AIExecutionPlan) ────
