...
export interface ResolvedBatchTask {
  ...
  /** 未命中但用户写了名字 → 落库时新建这个名字的事件线。 */
  eventLineCreateName: string | null;
  ownerId: string | null;
 ...
 * 宁可匹配不到走 graceful 降级(人落背景/事件线新建), 也绝不静默误关联。
 */
function matchByName<T>(name: string, list: T[], getName: (t: T) => string): T | null {
  const n = norm(name);
  if (!n) return null;
  return list.find((x) => norm(getName(x)) === n) || null;
}

/** 把一条解析后的任务, 依据本地名册解析成 id(纯函数)。 */
export function resolveBatchTask(task: ParsedBatchTask, dirs: BatchDirectories): ResolvedBatchTask {
  // 客户
  const client = task.clientName ? matchByName(task.clientName, dirs.clients, (c) => c.name) : null;

  // 事件线: 命中已有 → id; 有名字但没命中 → 记下待新建。
  const el = task.eventLineName ? matchByName(task.eventLineName, dirs.eventLines, (e) => e.name) : null;
  const eventLineCreateName = task.eventLineName && !el ? task.eventLineName : null;
  ...
  if (task.ownerName) {
    const m = matchByName(task.ownerName, dirs.members, (u) => u.fullName);
    if (m) {
      ownerId = m.id;
  ...
  for (const name of task.collaboratorNames) {
    const m = matchByName(name, dirs.members, (u) => u.fullName);
    if (m && m.id !== ownerId) {
