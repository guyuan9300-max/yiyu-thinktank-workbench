...
function createBrowserWorkbenchFallback(): Window['yiyuWorkbench'] {
  ...
  return {
    backendBaseUrl,
    setMiniMode: async (enter: boolean) => ({ mini: enter }),
...
// 让 src/renderer 真消费 V3 M1-M3 endpoint, 满足用户甲硬门槛 9 "前端不可见不算"
// ──────────────────────────────────────────────────────────────────────
...
// ──────────────────────────────────────────────────────────────────────
// 用户甲 5/24 大型任务 · 组织搭建中心机器人同事 API wrapper
...
export interface BotMemberRecord {
  ...
  /** 用户甲 5/24: 身份启动密钥相关 (db 只存 hash, 这里只暴露 prefix 等 metadata) */
  token_prefix?: string;
...
// [B] 5/25 PM (用户甲 path C) · 双重归属复盘 · 2 个 endpoint
 ...
 *   const bundle = await fetchClientFactBundle('client_demo_alpha');
...
// 用户甲 5/26 拍板: 7B 模型秒级识别 + 识别不了不挂 (不做 keyword 兜底).
...
// ── AI 工作指令 (用户甲 5/24 §4): 复用 A 已写的 resolveBotByHandle /
