...
// 用户甲 5/24: 机器人同事 — 直接复用弹窗组件, 不挂底部抽屉
...
type Props = {
  ...
  /** 用户甲 5/24 P1: 当前登录用户 — 创建机器人时作为 created_by + creator 审批人 */
...
export function OrganizationSetupCenter({
  ...
  // 用户甲 5/24: 添加机器人同事弹窗 (按部门触发, 记录该按钮属于哪个部门)
  const [botDialogDept, setBotDialogDept] = useState<{ id: string; name: string } | null>(null);
  // 用户甲 5/24 M1: 机器人同事数据源 — 全组织 active bot 一次拉, LeaderPicker 按部门过滤展示
  ...
  // 用户甲 5/24 M3: 选员工时同时清掉 role.holderBotId (员工/机器人持岗人互斥)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="overflow-hidden rounded-[32px] border border-[#DCE4FF] bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-gray-100 px-8 py-6">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* 用户甲 5/24: 添加机器人同事入口已挪到每个部门"添加岗位"旁边, header 不重复放. */}
            {activeView === 'tree' ? (
              <button
                type="button"
                      ...
                      <div key={department.id} className="flex items-center gap-10">
                        ...
                        <div className="relative flex flex-col gap-3">
                          {department.children.map((role) => {
                            ...
                            // 用户甲 5/24 M4: 持岗人解析 — 机器人优先, 否则反查 bindings
      ...
      {/* 用户甲 5/24: 添加机器人同事弹窗 (从每个部门"选员工"下拉里点 → 入口触发) */}
      {botDialogDept ? (
        <BotMemberFormDialog
          mode="create"
...
type LeaderPickerProps = {
  ...
  compact?: boolean;
  disabled?: boolean;
  /** 用户甲 5/24: 当下拉允许添加机器人同事时, 提供该回调 → 底部出现 "添加机器人同事" 行. */
  onAddBotMember?: () => void;
  /** 用户甲 5/24 M2: 机器人同事候选列表 (调用方按部门过滤后传入). 非空时下拉员工区下方加 "机器人同事" 区. */
            ...
            <>
              ...
              >
                ...
                <div className="max-h-[260px] overflow-y-auto">
                  ...
                  {/* 用户甲 5/24 M2: 机器人同事区 — 员工列表下方独立分组, uppercase 小标题, 每项前 [AI] 角标. */}
                  ...
                  {/* 用户甲 5/24: 下拉底部加 "添加机器人同事" 行 — 跟人类同事并列在同一序列里.
