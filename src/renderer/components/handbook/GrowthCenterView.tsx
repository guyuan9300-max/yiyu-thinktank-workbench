      ...
      {sortedEntries.length > 0 ? (
        <div className="gc-insight-grid">
          {sortedEntries.map((entry) => {
            const isAI = entry.authorUserName?.includes('大周') || entry.authorUserName?.includes('成员甲') || entry.authorUserName?.includes('花花') || entry.authorUserName?.includes('罗茜茜') || entry.sourceType?.includes('ai');
