/**
 * AICommandModal · AI 工作指令入口 (用户甲 5/24 §5)
 *
 * 替代 SmartTaskParseModal 入口. 内含 2 mode:
 *   - quick_task: 走原 ai-parse 链路 (保留原能力, 用户甲原则一)
...
type AICommandModalProps = {
  ...
  /**
   * 用户甲 5/25 真用反馈: plan inline approved 进 submitted 时, 立刻把 planId 抬到 App level,
...
export function AICommandModal({
  ...
  // 用户甲 5/25 安全边界: 每个任务以客户隔离, 必须强制有客户.
  ...
  // ── M7: inline @mention picker 状态 (用户甲 5/25: 微信群那种弹窗) ──
  ...
  // ★ open 时拉 active 机器人列表 (用户甲 5/24: @ 下拉支持)
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
  ...
  // ── 选 AI 同事: 自动 prepend "@<handle> " 到 textarea start (用户甲 5/24, 旧入口) ──
  ...
  // ── M7: inline mention 检测 (用户甲 5/25 微信群弹窗体验) ──
  ...
  const handleParse = async () => {
    ...
    // 用户甲 5/25 真用 bug: toggle=quick_task 但用户输入带 @机器人时, 强制 quick_task,
    ...
    if (!parsed.bot_handle) {
      setErrorMessage('AI 工作指令需要 @机器人同事. 例如: @成员甲 帮我...');
      setStage('input');
      return;
    }
    ...
    setStage('parsing');
    try {
      // 用户甲 5/25 PM 真用反馈: regex 解析跟不上自然口语, 必须 LLM 真解析.
  ...
  const handleSubmitPlan = async () => {
    ...
    // 用户甲 5/25 P0-6 真用 bug: 同一指令跑了 3 轮 = 15 份文档. 提交前查 in-flight,
    ...
    try {
      ...
      // 用户甲 5/25 安全边界: 每个任务以客户隔离, 没客户禁止提交 (防跨客户污染).
      ...
      // 用户甲 5/25 真用 bug fix: plan_executor 拿 steps:[] → total=0 → failed.
      ...
      const result = await createBotTaskPlan(bot.bot_member_id, {
        ...
        human_initiator_id: currentUserId || 'user_admin_demo',
      });
      ...
      // 用户甲 5/25 真用反馈: approved 路径下, 立刻把 planId 抬到 App,
      ...
      <div className="w-[min(720px,92vw)] max-h-[88vh] overflow-y-auto rounded-2xl bg-white shadow-[0_24px_70px_rgba(15,23,42,0.18)] ring-1 ring-inset ring-gray-100">
        ...
        {stage === 'input' && (
          <div className="px-6 py-4 space-y-3">
            ...
            {/* M7: textarea + inline @mention picker — 用户甲 5/25 微信群体验 */}
        ...
        {/* Stage: parsing — LLM 真解析中 (用户甲 5/25 PM: 本地 qwen2.5:7b ~5s) */}
        {stage === 'parsing' && (
        ...
        {stage === 'bot_resolved' && bot && parsed && (
          <div className="px-6 py-4 space-y-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-0.5 text-[10.5px] text-gray-500">
                  {parsed.steps.length > 0
                    ? `共 ${parsed.steps.length} 步, 请逐步核对成员甲是否理解准确`
                    : '没识别出明确步骤, 退回显示原文 (成员甲可能误解任务结构)'}
                </div>
              </div>
                    ...
                    <div className="px-3 py-2 space-y-1.5">
                      <div className="flex gap-2">
                        ...
                        <span className="text-[12px] text-gray-800 leading-relaxed">
                          {step.action || <span className="text-gray-400 italic">(未识别 — 成员甲可能误解)</span>}
                        </span>
                      </div>
                      <div className="flex gap-2">
            ...
            {/* 用户甲 5/25 安全边界: 关联客户必选 (任务以客户隔离, 没客户=跨污染). */}
            ...
            <div className="flex items-center justify-between pt-1 gap-2">
              ...
              <div className="flex items-center gap-3">
                ...
                {/* 用户甲 5/25 真用反馈: 这页已经清楚, 直接确认执行, 不再多一步 plan_preview */}
                <button
                  type="button"
                  ...
                  title="点击 = 我作为审批人当场确认这个流程, 成员甲立刻开始执行"
                >
        ...
        {/* Stage: submitted - 用户甲 5/24 真用反馈: "确认这个流程 = 审批本身; 任务结束要类似邮件通知" */}
...
// ── M10 (A, 2026-05-25) · plan 执行进度展示 (用户甲 5/24: "复杂任务 5-30 min 必须看得到进度") ──
...
// ── M7: inline @mention picker (用户甲 5/25 微信群体验) ──
// 注: inline 在本文件, 不单独建文件 (用户甲 hard rule).
