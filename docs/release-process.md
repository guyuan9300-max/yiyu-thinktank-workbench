...
> 2026-07-13 起，益语智库官网是新版客户端唯一版本真相源和安装包下载入口。未连接组织读取公开版；已连接组织优先读取定向版，否则回落公开版。旧 TOS 章节仅用于首个迁移版本双发，不再代表新版客户端运行规则。
...
- 新版客户端每 24 小时最多自动查询官网一次，只发送站内通知，不静默下载或安装。
- 用户在“关于本软件”中查看与目标版本严格对应的说明，主动下载安装包并按系统提示覆盖安装。
- 官网后台上传并校验安装包后，点击发布即生效；发布后检查 JSON 版本接口、HEAD、Range/206 与 SHA512。
- 包含新链路的首个迁移版本仍需官网与 TOS 双发一次，供旧客户端升级；完成迁移后不再更新 TOS。
