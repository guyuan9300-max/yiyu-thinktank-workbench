...
| **R4** | shared/types.ts +12 类型 + api.ts +5 个 client + AIReportGeneratorModal.tsx 700+ 行（4 阶段编排）+ App.tsx 集成（事件线详情顶部"AI 报告"按钮 + 下载浮层）+ tsc 0 error | 60 调用 | ✅ 编码完成（待用户 UI 验收） |
...
### 1. `src/shared/types.ts` · +12 类型（snake_case，与后端 Pydantic 对应）

`ReportChartKind` / `ChartHint` / `SectionPlan` / `ReportBlueprint` / `ReportCitationType` / `CitationRef` / `GeneratedChart` / `SectionContent` / `DraftBlueprintRequest` / `DraftSectionsRequest` / `ReportRunStatus` / `ReportSectionStatus` / `ReportFileFormat` / `ReportRunSummary`。

> ⚠️ 项目其他类型用 camelCase，但后端 Pydantic 模型 `ReportBlueprint` 等用 snake_case 且没加 `alias_generator`。R4 先保持 snake_case 与后端对齐；未来 R5 阶段考虑改后端用 `alias_generator=to_camel` 统一。

### 2. `src/renderer/lib/api.ts` · +5 个 client 函数

| 函数 | 调用 |
|---|---|
| `draftReportBlueprint(payload)` | POST `/api/v1/reports/draft-blueprint` |
| `draftReportSections(runId, payload)` | POST `/api/v1/reports/{id}/draft-sections` |
| `renderReport(runId, format)` | POST `/api/v1/reports/{id}/render?format=...` |
| `getReportRun(runId)` | GET `/api/v1/reports/{id}` |
| `getReportFileDownloadUrl(runId, format) → string` | URL helper for `<a download>` |

### 3. `src/renderer/components/reports/AIReportGeneratorModal.tsx`（新，700+ 行）

4 个阶段状态机（`Phase`）：
- `intent`：录入意图 → 期间/读者/基调（默认本季度 + "客户决策层"+"客观、克制、可执行"）
- `reviewing-blueprint`：豆包返回 blueprint 后展示标题/副标题/类型/受众/基调/置信度/推导主题/N 节预览（含 chart_hints + data_sources tag）+ `open_questions_for_human` 黄色 callout
- `drafting-sections`：每 3 秒 `getReportRun()` 轮询 `sections_status`，每节 row 实时显示 spinner/check/X 三态 + 顶部百分比进度
- `rendered`：3 个 file row（docx/md/pdf），ready 状态下"下载"按钮、未生成状态下"生成"按钮
- `failed`：错误信息 + "重新开始"按钮

子组件 8 个：`Header / ErrorBanner / IntentFormBlock / Field / BlueprintReviewBlock / SectionPlanCard / DraftingProgressBlock / ProgressBadge / SectionRow / SectionStatusIcon / RenderedBlock / FileRow / FailedBlock`。

样式：Tailwind v3 + lucide-react 图标。无新依赖。

### 4. `src/renderer/App.tsx` · 最小集成（4 处改动）

| 行 | 改动 |
|---|---|
| 448 | `import AIReportGeneratorModal from './components/reports/AIReportGeneratorModal';` |
| 8608 | `const [aiReportTarget, setAiReportTarget] = useState<{ id: string; name: string; clientName?: string } \| null>(null);` |
| 14700-14720 | 事件线详情顶部的"汇报预览"按钮旁加"AI 报告"按钮（Sparkles 渐变蓝），点击 setAiReportTarget |
| 14913-14941 | 渲染 `<AIReportGeneratorModal>`，`onDownload` 通过 `URL.createObjectURL` + `<a download>` 触发浏览器下载，并 `flash('success', '已下载 ...')` |

### 5. tsc 全量类型检查 0 error。

### 6. UI 验证路径（请用户人工跑一遍）

1. Electron 应用启动 → 进入「日程任务」tab
2. 找到一个有 activities 的事件线（推荐"日慈战略陪伴"，24 条 activities）
3. 点开事件线卡片 → 顶部右上角新增「AI 报告」按钮（带 ✨ 图标）
4. 点击 → 弹出 Modal「步骤 1/4 · 设置报告意图」
5. 填意图（"给客户的 Q1 战略陪伴报告"）→ 点「开始草拟骨架」
6. 等 30s → 进入「步骤 2/4 · 审阅骨架」，看 LLM 推的 4 节、置信度、3 条 open_questions
7. 点「确认骨架，开始起草」→ 进入「步骤 3/4 · 起草章节」
8. 看每节 spinner → green check 逐个变化（约 2-4 分钟，并行 max_workers=4）
9. 全节 done 后点「生成 docx」→ 进入「步骤 4/4 · 已完成」
10. 点「下载」→ 浏览器保存对话框 → docx 落到 ~/Downloads/

### 7. 已知风险更新

| 风险 | 状态 |
|---|---|
| Modal 在大型 App.tsx 内集成 | ✅ 最小侵入 4 处改动，无 TS 错误 |
| BackgroundTask 期间用户关 Modal | ⚠️ 后台 task 仍跑完；下次打开 Modal 会丢失上下文。R5 加"历史报告"列表恢复 |
| snake_case 与项目 camelCase 不一致 | ⚠️ 仅 reports 子模块；R5 考虑迁移 |
| 下载浮层 vs Electron saveFileAs | ⚠️ 当前走浏览器下载，文件落 ~/Downloads/；想存指定路径要走 Electron saveFileAs，下个迭代加 |

---

## 下次 checkpoint

R4 完成 → commit → 报告给用户 → 让用户在 Electron UI 跑一遍验收 → 通过后进入 **R5 多主题样例 + 使用手册** 收官。

**R5 范围预告：**
- 用 3-5 条不同主题的事件线（战略陪伴 / 产品复盘 / 募款汇报 / 项目结项 / 危机响应）跑 R1→R3 完整流程
- 对比生成的 docx 与沙箱样例风格（验证"主题感知"假设）
- 写《报告生成器使用手册.md》：UI 操作指引 + LLM 调参建议 + 故障排查 + 限制说明
- 可选优化：snake_case → camelCase 统一、历史报告列表、saveFileAs 集成
