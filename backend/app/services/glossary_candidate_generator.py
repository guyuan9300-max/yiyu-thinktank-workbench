...
例:
  · "日慈"/"日慈基金会"/"日慈公益基金会" → canonical_name="日慈基金会", aliases=[其他]
...
def _collect_project_candidates(db: Database, client_id: str) -> list[dict]:
    """主动 grep atomic_facts 找『心盛计划/心灵魔法学院』这种项目名 — 不受 mention_count/attribute 频度过滤."""
    rows = db.fetchall(
        """
    ...
    for r in rows:
        ...
        # 3) subject 含项目名子串 (例: "建议以心灵魔法学院中" → 心灵魔法学院)
