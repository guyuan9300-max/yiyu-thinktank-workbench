...
_EXTRACTION_SYSTEM_TEMPLATE = (
    ...
    "你必须**穷尽**式扫描全文，**任何**与字段相关的描述都要提取，不要因为格式不标准就放弃。\n"
    "\n"
    ...
    "\n"
    "规则：\n"
    "1. 资料中**完全没有**的字段（穷尽全文都搜不到任何线索），才输出 value=\"待补全\", confidence=0\n"
    "2. value 用原文表述（不要意译），原文怎么写就怎么记，**不要求严格格式**\n"
    "3. **多组件薪资必须合并**：如合同写「基本薪资5000，岗位补贴3000，绩效2000，其他补贴1000」，\n"
    "   regular_salary 应输出「基本薪资5000元+岗位补贴3000元+绩效2000元+其他补贴1000元（合计约11000元/月）」\n"
    "   confidence=0.95（不要因为没明确写「11000元/月」就填「待补全」）\n"
    "4. **试用期可以是时间段或月数**：\n"
    "   - 合同写「试用期为2023年6月15日至9月14日」→ probation_period=「3个月（2023-06-15 至 2023-09-14）」confidence=0.95\n"
    "   - 合同写「试用期3个月」→ probation_period=「3个月」confidence=0.95\n"
    "   - 合同写「无试用期」→ probation_period=「无试用期」confidence=0.95\n"
    "5. **试用期薪资也要尝试**：\n"
    "   - 合同写「2024.2.18-2024.5.17：2800 元；2024.5.18 起：3500 元」→ probation_salary=「2800 元」，regular_salary=「3500 元」\n"
    "   - 合同只写转正薪资未提试用期工资 → probation_salary=「待补全」（确实没有才填）\n"
    "6. **工作地点**优先用合同明确条款；如无明确「工作地点」字段，可用甲方地址作为推断（confidence=0.7），\n"
    "   并在 evidence 注明「合同未明确工作地点，按甲方注册地址推断」\n"
    "7. **部门**：合同里如「岗位」隐含部门（如「项目部主任」隐含项目部）应填，confidence=0.7\n"
    "8. **合同类型**：默认推断「全日制固定期限劳动合同」（若有起止日期），confidence=0.7；\n"
    "   除非合同明确写「劳务派遣/兼职/实习」等\n"
    "9. evidence 是支持该字段值的原文片段，截取 30-100 字\n"
    "10. confidence: 资料里明确写明=0.95；从合同结构推断=0.7；无任何依据=0\n"
    "11. **禁止凭空捏造**：必须有原文支持；但**禁止过度保守**：能从原文推断的字段不要写「待补全」\n"
)
...
_UNIVERSAL_EXTRACTION_SYSTEM_TEMPLATE = (
    ...
    "\n"
    "规则：\n"
    "1. **完全不存在**的字段（穷尽搜索都找不到任何线索）：value=\"-\"（不是「待补全」），confidence=0\n"
    ...
    "4. **key_amounts 必须穷尽提取**：扫描全文所有数字+货币单位（元/万元/人民币/¥），\n"
    "   合同薪资（基本+岗位+绩效+补贴）、项目预算、罚款、赔偿金、押金、保证金……一个都不能漏。\n"
    "   例：合同写「基本薪资5000，岗位补贴3000，绩效2000，其他补贴1000，月基本工资100%」\n"
    "   → key_amounts=「5000 元基本薪资,3000 元岗位补贴,2000 元绩效,1000 元其他补贴」\n"
    "5. **key_dates 必须穷尽**：合同期限起、止、签订日、试用期起止、续约通知日等所有日期\n"
    "6. **key_people 必须穷尽**：甲方代表人、乙方/员工、参会人、签字人等所有人名，附身份\n"
    "7. **key_locations**：合同地址、工作地点、项目地点、注册地、收货地址等所有地点\n"
    "8. 长文本字段（summary）：150-250 字，含**核心事实**+**关键判断**\n"
    "9. 短文本字段（main_purpose / title_inferred）：≤40 字\n"
    "10. **不要编造**：原文没有的内容必须写「-」；但**禁止过度保守**：原文有的内容必须提取，不要漏\n"
    "11. evidence 字段可以留空（universal 解构不强制溯源，节省 token）\n"
    "12. confidence: 资料里明确写明=0.95；推断/概括=0.7；不存在=0\n"
    "13. document_kind 用中文（如「员工合同」「会议纪要」「财务报表」「项目方案」「资讯简报」「录音转写」「笔记」「服务协议」「捐赠协议」「其他」）\n"
)
