...
def _looks_like_official_homepage(url: str, title: str, client_name: str) -> bool:
    ...
    严格启发式 (基于实测对C项目误判 163.com/sohu.com 导致噪音):
...
def _expand_client_name_candidates(client_name: str, ai: Any | None = None) -> list[str]:
    ...
    背景: 实测发现 7 客户中 3 个 (D组织/士平/B客户) 用注册简称在搜索引擎上找不到内容,
    但用全称 "广东省D组织公益基金会" 可以. 客户在数据库里录入名字时往往用日常简称,
    爬虫层应该有能力补全成"官方注册可能的全称"集合, 用全称去搜.

    机制 (不硬编码):
      1. 原名直接保留 (兼容已知客户)
      2. 用 LLM 推测可能的官方注册全称, 输出多个候选 (例: D组织 → 广东省D组织公益基金会 / 上海D组织慈善基金会)
...
def discover_authority_seeds(
    ...
    *,
    ...
    实测对"D组织/士平/B客户"这种名字模糊客户, 从 0-4 篇 → 期望 15+ 篇.
    """
    name = (client_name or "").strip()
    ...
    # 客户名扩展 (G 任务): "D组织" → ["D组织", "广东省D组织公益基金会", ...]
