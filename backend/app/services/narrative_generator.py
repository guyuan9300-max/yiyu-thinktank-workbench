...
DIMENSION_BRIEF = {
    ...
    "people": (
        ...
        "\n  数据来源: entities (person, mention_count 高) + tasks.owner_name + 业务上下文 (用户甲=益语 CEO)"
    ...
    "next_steps": (
        ...
        "\n  "
        "\n  示例(口语化的写法, 仅参考语气):"
        "\n    战略层: A组织眼下最关键的不是再加新业务, 而是把团队重组后的管理真空补上."
...
SYSTEM_PROMPT = """你是已经跟这个项目走了**半年**的高级战略顾问, 写的是给项目最高负责人 (用户甲, 益语 CEO) 看的**内参**。

============================================================
== Layer 0 · 项目关系定位 (基础前提, 写每一层之前必须先想清楚) ==
============================================================

益语智库 = **战略咨询公司** (你/用户甲所在的机构, 主营『战略陪伴』这种顾问服务)
...
============================================================
== ★★★ 关键变更事实必读 (用户甲 5/22 P1.3 内化规则) ==
============================================================

凡是 atomic_facts 含下列 attribute 类型的事实, **必须自然引用到对应 Layer 中**, 不要遗漏:

1. **人事变更** (attribute 含: 职务 / 角色 / 新任 / 接任 / 卸任 / 升任 / 调岗 / 离职)
   → 必须在 Layer 4 (people) **人物身份段** 引用
   → 例: "张真.新任职务=法人代表" → Layer 4 必写 "张真目前担任法人代表..."

2. **项目合并 / 重组** (attribute 含: 合并 / 重组 / 拆分 / 改名 / 整合)
   → 必须在 Layer 3 (business_intro) **业务结构段** + Layer 5 (timeline) **关键转折** 同时引用
   → 例: "兴盛+心理魔法学院.合并重组=项目部" → Layer 3 写业务现状, Layer 5 写转折时间

3. **产品 / 项目命名** (attribute 含: 类型 / 命名 / 启动 / 新项目)
   → 必须在 Layer 1 (essence) **项目矩阵** + Layer 3 (business_intro) **核心业务** 引用
   → 例: "安心妈妈.资源联动对象=...." → Layer 1+3 都要 mention 安心妈妈

4. **战略 / 风险变化** (attribute 含: 风险 / 战略 / 转向 / 卡点)
   → 必须在 Layer 6 (next_steps) **战略层 / 风险对冲** 引用

**判定方法**: 每写一段叙事前, 扫一遍 atomic_facts, 哪些 attribute 是上述类别 → 必须 mention 在对应 Layer.

**为什么这是元规则**:
- 用户感知"AI 真读懂"的核心 = AI 写叙事时引用了关键变更
- 没 mention 关键变更 = 用户看完 6 段觉得"AI 漏了重要事实"
- 5/19 张真会议 7 关键事实 (法人/理事长/强哥/秘书长/兴盛/心理魔法学院/安心妈妈) 全在 atomic_facts, 必须 mention
...
(b) **顾问判断** — 你作为顾问对 fact 的解读, 用客观书面语表达, **不要用第一人称套话**
    **仅在需要判断/推论的层使用** (cooperation/next_steps), 不要在事实介绍层 (essence/business_intro) 滥用
    ...
    ❌ 错 (essence 层做主观判断): "A组织这家机构应该重点关注..." ← essence 层不该做判断
    ✅ 对 (cooperation/next_steps 层做客观判断): "从合作 N 个月的进展看, 益语属于A组织长期标杆服务型, 优先级较高"
...
(c) **澄清问题** — fact 不够判断时, 不要替客户回答, 把问题放在 openClarifications
    例: openClarifications: ["18-24 岁是不是A组织核心服务对象? 还是只是行业研究背景?"]
    要点: 顾问不知道的, 直接问, 不替客户编
...
def build_user_prompt(bundle: ClientFactBundle) -> str:
    ...
    # ★★★ M-C.3 强制 mention 清单 (用户甲 5/22 autonomous loop, V2.1 lab 改)
    ...
    if _critical_facts:
        lines.append("\n# ★★★ 强制 mention 清单 (用户甲 5/22 M-C.3 钦定 — 漏一个视为 LLM 失败)")
    ...
    lines.append("  例 1: ❌ 『18-24 岁抑郁峰值, **因此**A组织选这个赛道』")
    lines.append("        ✅ 『fact #X 显示 18-24 岁抑郁峰值是行业事实, 通常会是公益机构选服务对象的依据 — 但A组织是不是真把 18-24 岁锁定, 需要澄清』")
    lines.append("        ✅ openClarifications: [『A组织核心服务对象是 18-24 岁? 还是包括其他年龄段?』]")
    ...
    lines.append("    ✅ 对: 『A组织总部位于广州』(sourceId 放 references 数组, 不在 narrative 文本里)")
    ...
    lines.append("**这套元规则适用于任何客户 (A组织/黔行/C项目/华润...). 它防的是结构, 不防具体词。**")
    lines.append("")
    ...
    # R4-P1-1 (用户甲 5/23 R4-P1 钦定) · 注入 R4 字段到 narrative prompt
    ...
    if _r4_contracts or _r4_historical or _r4_files or _r4_gaps or _r4_external:
        lines.append("\n# ⭐⭐⭐⭐⭐ R4 公司大脑 (R3 派生的 5 类新证据 — 用户甲 R4-P1 钦定必用)")
...
def upsert_commitments_from_narrative(
    ...
    # 机制化兜底: 即使 LLM 不听 prompt 输出"高老师（D组织方）", 写入前剥掉括号里的"方"字 —
