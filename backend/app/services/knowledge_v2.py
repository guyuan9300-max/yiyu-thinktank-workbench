...
def _resolve_team_context_for_async_worker(db: Any) -> dict[str, str]:
    ...
    try:
        ...
        if not org_id:
            return {}
        # 跳过系统账号 (user_admin / @example.org), 取真人 admin
        admin_row = db.fetchone(
            ...
            WHERE primary_role = 'admin'
              ...
              AND (email IS NULL OR email NOT LIKE '%@example.org')
            ORDER BY id LIMIT 1
            """,
            (org_id,),
        )
...
def ingest_document_knowledge(
    ...
    *,
    ...
    # 用户甲 5/26: AI 主动生成的成品文档 (导出答案 / 答案备忘 / 咨询沉淀 / 模板填写) 真不参与
...
def fanout_document_to_event_lines(
    ...
    for el in event_line_rows:
        ...
        # 这样像「A组织战略合作.docx」能匹配「A组织战略陪伴」事件线。
