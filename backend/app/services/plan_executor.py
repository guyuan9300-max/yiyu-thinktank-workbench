...
背景:
  用户甲 5/24 真用 @成员甲 给安然下了 3 件指令, plan inline approved 走通了, 但之后
  成员甲 0 动作 — 因为 create_ai_task_plan 只记录 plan, 不执行. B 在 46-B 求我接.

设计原则:
  · ExecutorRegistry — tool_name → handler 映射, 加新 handler 不动主流程 (用户甲
    硬约束: "不要在 prompt 里写死流程")
  · 异步执行 — FastAPI BackgroundTasks, 不阻塞 approve 返回 (用户甲 5/24 "5-30 min
    任务必须看得到进度", M10 进度可视化挂在这上面)
  · 全 agent_run_log 留痕 — actor_type=internal_ai_agent, actor_id=bot.actor_id,
    每步 start + complete/failed 写两条 (用户甲 self_approve 硬禁止: actor 是 bot
...
def markdown_to_docx(md_text: str, out_path: Path, *, title: str = "") -> Path:
    ...
    for raw in lines:
        ...
        # **粗体** 单独成行 → 当 Heading 2 (LLM 真实输出方式, 用户甲 5/25 PM 反馈)
...
- index: 1, 2, 3, ...
- action: 成员甲要做的具体动作 (动词短语, 20-60 字). 例: "写一份A组织客户背景档案 (含组织介绍 + 项目清单)"
- basis: 这一步基于什么 (上下文/参考/前置 step 产出, 10-80 字). 例: "数据中心已有A组织合同 2 份 + A组织官网公开资料 + 益语 2025-2026 服务记录"
...
6. 如果用户提到具体客户 (如"A组织"), action 里要带客户名, 不要写成"撰写介绍"这种泛指
...
def _parse_steps_via_ollama(system_instruction: str, prompt: str, timeout: float = 30.0) -> dict | None:
    ...
    用户甲 5/25 真测: qwen2.5:7b 4.8s / qwen2.5:14b 18s, 7b 已经够用.
    选 7b 是为了"秒级解析" 体验.
...
def parse_steps_with_llm(text: str, today_str: str = "") -> dict:
    ...
    用户甲真用反馈: regex 拆 step 跟不上自然语言. 必须 LLM 真理解.
...
# ── [B] 5/25 PM (用户甲 path D) · AI 同事跟人走一套接口 ─────────────────────
# 设计: bot 做的事自动生成 task + 自动完成 + 自动复盘
# 让用户甲在重点主线/日历/复盘直接看到, 不需要独立 AI 视图.
...
def _bot_write_self_review(
    ...
    """成员甲自动写复盘 — 思考 + 不足 + 下一步. 不调 LLM (太慢), 模板式生成.

    P1 (用户甲拍可升级): 调本地 qwen2.5:7b 让成员甲真写反思.
    """
    # 基于真数据生成反思 (不是空话)
...
def _write_review_entry(
    ...
    """给某 user 的本周复盘加一条 task entry (成员甲自动复盘走这里)."""
...
# ── [B] 2026-05-25 PM · AIService singleton (用户甲拍板"真接 LLM, 不模拟") ───────
...
# ── 中文 action → 文档类型 enum (用户甲虚构客户/真客户都覆盖) ─────────────────
...
def _step_to_subtask(step: dict, idx: int, plan: dict) -> dict:
    ...
    # 推 tool 名: module/action 映射 (用户甲 hard rule: 不写死流程, 但路由表是合理的)
...
def _handler_documents_generate(
    ...
    """[B] 2026-05-25 PM 重写 (用户甲拍板"从底层架构修, 不要硬编码模拟"):
    ...
    # 用户甲原话: "写任何资料, 一定是优先基于数据中心和工作台的内容,
    ...
    if ai is not None and hasattr(ai, "_qwen_generate"):
        ...
        system_instruction = (
            f"你是 {bot.get('display_name', '成员甲')}, 益语智库 {bot.get('department_name', '战略发展部')} 的 AI 同事. "
            f"你的工作风格是严肃专业的咨询顾问. "
            f"严格规则:\n"
        ...
        # 拼参考材料块 — 用户甲 5/25 PM 真用反馈: 必须真拉内容, 不只统计
        ...
        prompt = (
            ...
            f"{evidence_section}\n\n"
            f"# 数据源使用硬规则 (用户甲 5/25 PM 钦定, 必须严格遵守)\n\n"
            ...
            f"# Markdown 排版硬规则 (用户甲 5/25 真用反馈, 必须严格遵守)\n\n"
            ...
            f"   ```\n"
            f"   # 安然集团客户背景档案\n\n"
            f"   生成者: 成员甲 · 时间: {today_str} · 数据源: 用户指令上下文\n\n"
            f"   ## 集团历史沿革\n\n"
            ...
            f"   ```\n\n"
            f"# 其他硬规则\n\n"
            f"1. 文档开头一行 metadata (无 # 号): `生成者: {bot.get('display_name', '成员甲')} · 时间: {today_str} · 数据源: ...`\n"
        ...
        try:
            ...
            # 不抛, fallback 写一条 markdown stub 进 documents 表 (用户甲能看到失败原因)
            markdown = (
    ...
    # 4. 推 title (用户甲 5/25 反馈: 不要用 expected_result, 它是落点描述)
    ...
    # 5. P0-1 真出 .docx 文件 + 中文文件名 (用户甲 5/25 PM 反馈)
    ...
    # ── [B] 5/25 PM · path D · 成员甲跟人走一套接口 ──
    # 1. 给本次文档建一个 task (owner=成员甲, related=用户甲 协作者)
    # 2. 立刻 status='done' + completed_at (因为文档已生成)
    # 3. 给成员甲本周 review 加一条复盘 entry (它自己写反思)
    # 4. 同时给发起人 review 也加一条 (内容相同, 让用户甲进自己复盘也看到)
    ...
    try:
        ...
        # 2. 发起人 (用户甲) 本周复盘 — 同一 task 也出现在他的复盘里
        if human_initiator_id:
...
def _handler_tasks_create(
    ...
    """[B] 2026-05-25 PM 重写 (用户甲拍板"真填字段, 不要 plan_text 前 30 字").
    ...
    新逻辑:
      ...
      owner_id   = plan.human_initiator_id (用户甲真发起)
      owner_name = mirror_users 真查 (不再空字符串)
      creator_id = bot.id (成员甲建的, 不是匿名)
    """
    # 1. title - 用户甲 5/25 真用 bug: payload.expected_result = step.deliverable
    ...
    # [B] 5/25 PM (path D 用户甲洞察) · AI 当真同事 · 走人一样的接口:
    # · owner = 成员甲 (它真做这件事)
    # · 用户甲 = collaborator (日历/任务列表可见, 但归属权属人)
    ...
    def _do_insert_task(conn) -> None:
        ...
        # [B] path D: 加发起人为 collaborator (让用户甲在日历/任务列表看到这条会议)
        if human_initiator_id:
            conn.execute(
                """INSERT OR IGNORE INTO task_collaborators (
                    task_id, organization_id, user_id, full_name, email,
    ...
    # [B] path D: 成员甲自动写复盘 (会议类任务不 auto-done, 但要复盘"为什么建这个会议")
    try:
        review_note = _bot_write_self_review(
            bot_display_name=bot_display_name,
...
def _handler_smart_import(
    ...
    不抛异常, 标 noop_unsupported 让进度走完 (用户甲约束: 不卡死).
    """
