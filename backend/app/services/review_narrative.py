...
写作规则：
1. headline: 用一句话定性这一周，必须包含一个具体事实（如"测试机构A教师赋能完成Q1复盘，测试机构B创始人分歧待协调"），不要泛泛说"持续推进""取得进展"。
...
5. nextWeekHint: 必须是一个可执行的管理建议，包含具体的人/事/时间。例如"下周五前让高老师交品牌规划，否则测试机构AQ2节奏会延迟"。不要写"优先收束""聚焦存量"这种没有行动对象的空话。
