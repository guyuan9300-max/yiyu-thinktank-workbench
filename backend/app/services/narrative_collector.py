...
class ClientFactBundle:
    ...
    # 业务上下文 (益语方法论/用户甲身份/战略陪伴定义), 从 module_definitions DNA 抽
    ...
    # R4 P0-4 · 用户甲 5/23 钦定: 战略陪伴 6 段叙事必须读 R3 新表
...
def _collect_business_context(db: Database) -> list[BusinessContextSnippet]:
    """从 module_definitions DNA 抽『顾问默认常识』, 顺序优先抽核心益语方法论 + 用户甲身份."""
...
def _collect_atomic_facts(db: Database, client_id: str, limit: int) -> dict[str, list[AtomicFactRow]]:
    """拉 atomic_facts (用户甲 5/22 M-C.1: 放宽 status 过滤).
