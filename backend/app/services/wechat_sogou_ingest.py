...
def _extract_unique_keyword(client_name: str) -> str:
    ...
    搜狗对长 query "A组织" 会把"基金会"这个泛词单独命中, 导致 70%+ 污染.
    剥掉客户名末尾的通用后缀, 剩下的核心字符 (例 "A组织" / "南都" / "壹基金") 才是
...
def _stable_origin_id(*, title: str, author: str) -> str:
    ...
    title 在搜狗结果里是带空格分词的 ('A组织 公益 基金会 ...'), 这里 normalize 掉空格后再 hash.
    """
...
def run_wechat_sogou_ingest(
    ...
    *,
    ...
    extra_query_aliases: 客户的常用别名 (例 ['A组织', 'A组织']),
    ...
    # 关键: 抽客户名独特子串. "A组织" → "A组织" 防搜狗对"基金会"泛词污染.
    ...
    for query in queries:
        ...
        for result in page_results:
            ...
            if not sogou_url or "weixin.sogou.com" not in sogou_url:
                continue
            # 严格相关性: title 或 snippet 必须含独特关键词 ("A组织") 或客户全名/别名
