...
OUTPUT_PATH = Path("/sessions/nifty-zen-euler/mnt/yiyu-thinktank-workbench/docs/A组织战略陪伴报告-Q1-2026.docx")
...
def build():
    ...
    add_paragraph(
        doc, "本报告版权归A组织与益语智库共同所有；未经双方书面同意不得对外披露",
        alignment=WD_ALIGN_PARAGRAPH.CENTER,
    ...
    add_callout(
        doc,
        "本报告基于 2026-01-01 至 2026-04-30 期间A组织与益语智库的全部战略陪伴"
    ...
    add_paragraph(
        doc,
        "本季度A组织在「以关系重塑一切」韧性生态战略框架下推进四大核心项目，"
        "关键进展如下。"
    )

    add_heading(doc, "核心判断（本季度沉淀）", level=2)
    judgments = [
        "A组织四大核心项目已确立战略层级划分：第一曲线（心灵魔法学院）+ 第二曲线（心松松、心盛）+ 生态层（繁心计划）。",
    ...
    decisions_table = [
        ["心灵魔法学院 2026 节奏调整为「小步迭代」、暂不扩张", "2026-Q1", "A组织方主理人", "第一曲线整体", "执行中"],
    ...
    add_paragraph(
        doc,
        "A组织正处于「战略转型期」的关键时刻 —— 从过去依赖单次活动交付的传统公益模式，"
    ...
    short_term = [
        ["P0", "心松松飞书一期数字化完成", "A组织 + 飞书", "6 / 30"],
        ["P0", "心松松训练模块库完成第一版", "A组织心松松组", "6 / 30"],
        ["P1", "教师互助小程序上线", "A组织数字化组", "6 / 30"],
        ["P1", "心盛计划业务逻辑对齐收敛", "A组织心盛组", "5 / 31"],
        ["P1", "召开「关系资产量化标准」专题会议", "双方", "5 月内"],
        ["P2", "心松松现金牛产品 MVP 概念草案", "益语提初稿 + A组织确认", "6 / 15"],
    ]
    add_table(
        doc,
    ...
    add_data_source(
        doc,
        "数据源：entities WHERE type='person' AND client_id='A组织'"
    )
