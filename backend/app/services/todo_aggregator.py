...
def collect_all_todos(db: Any, client_id: str) -> list[UnifiedTodo]:
    ...
    try:
        ...
        for c in commitments:
            ...
            # 简单判方向: 含"益语/用户甲/...XX 老师" 类的算我方, 否则按字面
            our_side_keywords = ["益语", "用户甲", "顾老师", "陪伴", "智库"]
...
def _normalize_title_for_match(title: str) -> str:
    ...
    # 去掉所有标点和空白
    s = re.sub(r"[\s，。、；：:,.;\-—!\?\(\)\[\]【】《》""''「」]+", "", s)
    # 去掉 LLM 反复重复的客户名词缀 (例: '完成A组织一季度' / '完成A组织一季度' / '完成一季度' 归一为 '完成一季度')
...
def _dedupe_todos(todos: list[UnifiedTodo]) -> list[UnifiedTodo]:
    ...
    判定档位 (从严到宽):
      1. 强合并: 标题归一化后完全相等 → 合并 (不要求 owner/due 也等)
         覆盖场景: '完成A组织一季度...' vs '完成一季度...' 客户名词缀归一抹掉后相同
      2. 中合并: owner 归一化等价 (顾老师↔用户甲) + 标题相似 → 合并
