...
流程:
  ...
  4. 1 候选 → 高置信关联
  5. 多候选 → ambiguous, 创建 clarification (用户甲硬门槛 4 — 不强行判断)
...
def find_candidates(
    ...
    # 1b. contract_reference → contract_structures (用户甲 R3 核心: 合同结构搜)
    if ref.ref_type in ("contract_reference", "project_reference"):
        try:
            rows = db.fetchall(
    ...
    # 2a. amount_reference → contract_structures (用户甲 R3 钦定 — 金额优先查合同)
