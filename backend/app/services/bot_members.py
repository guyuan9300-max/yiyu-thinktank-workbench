"""[A] V3 Final · 组织搭建中心机器人同事 (用户甲 5/24 大型任务).
...
底线 hard_denies (用户甲 §4.4, 后端硬约束, 不放 UI):
...
# M8 (A, 2026-05-25): 默认 enabled 真口径 — 用户甲 5/24 真用反馈.
# 低风险动作默认开 (建 bot 即可用), 高风险动作默认关 (改 UI 再开).
# external_send.request / clarification_resolution.propose 默认关 (高风险, 需人显式赋能).
# inline_approval.allow_from_supervisor 默认开 (用户甲 5/24: "审批 = 确认流程, 提交那一刻就是审批本身").
...
# 不允许 inline approval 的高风险动作 (用户甲 §7.3)
...
def ensure_bot_schema(db: _DbLike) -> None:
    ...
    statements = [
        ...
        # P2 migration (用户甲 5/24 身份启动密钥): 旧表加 token 字段
        ...
        # P1 migration: 已有 bot_reporting_lines 表加 report_to_creator 列(用户甲 5/24)
        ...
        # [B] 5/25 PM (用户甲 path C) · org_members_v: 人 + bot 统一视图,
...
def create_bot_member(
    ...
    *,
    ...
    # 用户甲 5/24: 身份启动密钥. None 时自动生成 32 字符 URL-safe token.
    ...
    # 默认 enabled (用户甲 5/24 真口径): 显式传则按显式, 否则用 DEFAULT_ENABLED_CAPABILITIES.
    ...
    # 身份启动密钥 (用户甲 5/24): 必生成, 明文随返一次, db 只存 hash
    if not token_plain:
...
def get_bot_member(db: _DbLike, bot_member_id: str) -> dict | None:
    ...
    if not row:
        return None
    d = dict(row)
    # 用户甲 5/24 安全: 去掉 hash/salt 防泄露; 留 prefix + rotated_at + has_token bool 给前端展示
