...
def test_score_pair_edit_distance() -> None:
    """日慈科技 / 日兹科技：编辑距离 1，长度 ≥ 3 → 0.65"""
    sim, _ = _score_pair("日慈科技", "日兹科技")
    assert sim >= 0.65
...
def test_find_merge_candidates_isolates_by_type(conn) -> None:
    ...
    _seed_entity(conn, id_="e1", client_id="cli-A", type_="person", name="日慈")
    _seed_entity(conn, id_="e2", client_id="cli-A", type_="company", name="日慈科技")
    cands = find_merge_candidates(conn, client_id="cli-A")
    assert cands == []
...
def test_merge_entities_moves_references(conn) -> None:
    ...
    conn.execute(
        ...
        "VALUES ('t1', 'cli-A', 'e_merged', 'works_at', '日慈科技', 0.8, '2026-05-12', '2026-05-12')"
    )
    conn.execute(
...
def test_merge_entities_handles_object_side_of_triple(conn) -> None:
    ...
    _seed_entity(conn, id_="e_survive", client_id="cli-A", type_="company", name="日慈科技有限公司")
    _seed_entity(conn, id_="e_merged", client_id="cli-A", type_="company", name="日慈科技")
    ...
    conn.execute(
        ...
        "VALUES ('t1', 'cli-A', 'e_other', 'works_at', 'e_merged', '日慈科技', 0.8, "
