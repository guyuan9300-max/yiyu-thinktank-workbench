...
def test_layer1_string_eq():
    ...
    assert layer1_string_eq("A组织", "A组织") is True
    assert layer1_string_eq("  A组织  ", "A组织") is True
    assert layer1_string_eq("A组织", "广东省A组织") is False
...
def test_layer2_char_similarity_xinling_vs_xinli():
    ...
    # 长词 (心灵魔法学院 vs 示例课程项目)
    sim2 = layer2_char_similarity("心灵魔法学院", "示例课程项目")
    assert sim2 >= 0.83  # 共享 5 字 / 6 字


def test_layer2_负责人甲_vs_张铮():
    """另一个经典案例: 人名同音字."""
    from app.services.cross_source_check import layer2_char_similarity
    sim = layer2_char_similarity("负责人甲", "张铮")
    assert 0.4 <= sim <= 0.6


def test_check_with_xinling_vs_xinli():
    """综合 check: '心灵魔法学院' vs '示例课程项目' 应该进 clarify."""
    from app.services.cross_source_check import check

    result = check("心灵魔法学院", "示例课程项目")
...
def test_scan_client_for_cross_source_candidates():
    ...
    db.execute(
        "INSERT INTO atomic_facts (id, client_id, subject_text, attribute, value_text, status) "
        "VALUES ('f2', 'cli', '示例课程项目', '类型', '项目', 'active')"
    )
    db.execute(
        "INSERT INTO atomic_facts (id, client_id, subject_text, attribute, value_text, status) "
    ...
    # 至少能撞出 心灵 vs 心理 这对
    top = candidates[0]
    assert {top["text_a"], top["text_b"]} == {"心灵魔法学院", "示例课程项目"}
