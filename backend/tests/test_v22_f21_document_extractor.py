...
def _create_v2_document(
    ...
    *,
    ...
    file_name: str = "日慈测试.docx",
    kind: str = "docx",
    markdown_content: str = "日慈基金会成立于 2010 年。\n\n张真担任法人代表。",
) -> str:
    """建一个 v2_document 测试用"""
...
def test_contract_filename_maps_to_official_doc():
    """合同 → client_official_doc (顾源源 5/22 渠道驱动 fact)"""
...
def test_strategy_manual_maps_to_official_doc():
    ...
    assert _map_kind_to_source_type("pdf", "日慈战略执行手册.pdf") == "client_official_doc"
...
def test_extract_writes_facts_via_ingest_pipeline(db: Database, mock_ai_service, monkeypatch):
    ...
    doc_id = _create_v2_document(
        db,
        markdown_content="日慈基金会成立于 2010 年。\n\n张真担任法人代表。",
    )
    ...
    mock_payload = _make_llm_payload([
        ...
        },
        {
            ...
            "content_role": "fact",
            "layer": "L2",
            "evidence_text": "张真担任法人代表。",
            "time_anchor": None,
            "speaker_person_id": None,
            "confidence": 0.90,
            "reasoning_steps": ["段落 2 说张真担任法人代表"],
        },
    ])
    ...
    # 日慈 / 成立年份 / 5 维元数据完整
    assert rici_row["subject_text"] == "日慈基金会"
    ...
    # 张真 / 角色 / 5 维元数据完整
    assert zhang_row["subject_text"] == "张真"
    assert zhang_row["value_text"] == "日慈法人代表"
...
def test_extract_triggers_supersedes_when_update_keyword(db: Database, mock_ai_service, monkeypatch):
    ...
    db.conn.execute(
        """INSERT INTO atomic_facts (
            ...
            'af_old', 'c_rici', '日慈', '合同金额', '300 万', '300 万',
