...
def _insert_client(db: Database, client_id: str = "client_rici") -> None:
    db.execute(
        ...
        VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (client_id, "A组织", "A组织", "公益", "战略陪伴", "关注儿童韧性与教师支持网络。", "推进中", NOW, NOW),
    )
...
def _insert_event_line(db: Database, client_id: str = "client_rici") -> None:
    db.execute(
        ...
        VALUES(?, ?, 'project', 'active', ?, ?, ?, ?, ?, ?, 4, ?, 'A组织', ?, ?)
        """,
        (
            "event_teacher",
            "教师项目设计",
            "方案调整",
            "教师项目设计不完整，需要老师丙支持，不能由外部团队直接代偿。",
            "近期任务说明提到项目负责人笑雨项目设计能力较弱，需要老师丙支持。",
            "方案边界尚未定清。",
            "不要代偿项目组内部设计。",
            "先完成项目设计说明。",
            client_id,
...
def _insert_task(db: Database) -> None:
    ...
    db.execute(
        ...
        (
            "task_teacher",
            "教师项目方案调整",
            "任务说明：笑雨项目设计能力偏弱，需要老师丙支持，但外部团队不能代替项目组完成设计。",
            "负责人设计能力不足",
            "老师丙先给出项目设计边界",
            "不要代偿",
            NOW,
...
def _insert_dna(db: Database, client_id: str = "client_rici") -> None:
    db.execute(
        ...
        (
            client_id,
            "A组织希望从单个项目交付转向儿童韧性生态建设，教师支持网络是重要场景。",
            "A组织希望从单个项目交付转向儿童韧性生态建设，教师支持网络是重要场景。",
            "从项目交付转向韧性生态。",
            NOW,
        ),
...
def test_context_filters_noise_prompt_docs_and_keeps_event_task_signal(tmp_path: Path) -> None:
    ...
    _insert_task(db)
    _insert_document(db, document_id="doc_core", title="教师项目会议纪要.md", markdown="项目设计、负责人、老师丙支持、不要代偿。")
    _insert_document(db, document_id="doc_dup", title="教师项目会议纪要_202604281010.md", markdown="项目设计、负责人、老师丙支持、不要代偿。")
    _insert_document(db, document_id="doc_noise", title="A组织报销发票.jpeg", markdown="发票 报销 测试附件。")
    ...
    assert "A组织报销发票.jpeg" not in selected_titles
