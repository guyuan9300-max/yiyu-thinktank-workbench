...
def test_find_contract_routes_to_file_search() -> None:
    route = route_workspace_query(prompt="找A组织示例团队的合同", client_id="client_1")
...
def test_contract_summary_stays_answer_workflow() -> None:
    route = route_workspace_query(prompt="A组织咨询合同主要内容是什么", client_id="client_1")
...
def test_contract_prompt_boosts_contract_original_evidence() -> None:
    groups = _target_document_groups_for_prompt("A组织咨询合同主要内容是什么")
    contract_item = EvidenceItem(
        id="ev_contract",
        title="A组织咨询合同（0907）.docx",
        excerpt="甲方为A组织，乙方为示例团队文化传播有限公司。",
        sourceType="raw_document",
        path="/tmp/A组织咨询合同（0907）.docx",
