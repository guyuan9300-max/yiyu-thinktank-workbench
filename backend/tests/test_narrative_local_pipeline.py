...
def _seed_minimum(db: Database) -> None:
    ...
    db.execute(
        ...
        (CLIENT_ID, CLIENT_NAME, "A组织", "", "client", "", "active", "#5B7BFE", now, now),
    )
    # entities · 5 人物 + 3 日期 + 2 金额
    entity_rows = [
        ("ent_1", "person", "老师乙", "老师乙", 93),
        ("ent_2", "person", "老师丙", "老师丙", 16),
        ("ent_3", "person", "负责人甲老师", "负责人甲老师", 6),
    ...
    db.execute(
        """INSERT INTO event_lines(id, name, kind, status, stage, summary, intent,
           ...
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (
            "el_1", "A组织战略陪伴", "project", "active", "discovery",
            "为A组织做 6 个月组织诊断 + 干部培训", "客户对成功的定义是看到干部差在哪",
            "等负责人甲发时间规划", "4月初确定 4 个候选方向", "5月15日前出 visual story 提纲",
            3, CLIENT_ID, "[]", now, now,
        ),
    )
    db.execute(
        ...
        ("act_1", "el_1", "meeting", "doc_4_12", "2026-04-12T10:00:00Z",
         "管理员甲", "负责人甲秘书长见面纪要", "负责人甲明确支持品牌升级, 徐总监经办", "{}"),
    )
    ...
    db.execute(
        ...
        (
            ...
            "负责人甲秘书长见面纪要.md", "meeting_notes", "completed",
            "4/12 负责人甲秘书长见面... 同意品牌升级方向...",
            now, now, "hash_1",
        ),
    )
...
def test_collector_picks_up_real_persons_dates_money(db: Database) -> None:
    ...
    assert "老师乙" in person_names
    assert "负责人甲老师" in person_names
    assert any(p.name == "老师乙" and p.mention_count == 93 for p in bundle.persons)
...
def test_prompt_contains_persons_dates_money_atomic_facts(db: Database) -> None:
    ...
    # 关键人物
    assert "老师乙" in prompt
    assert "负责人甲老师" in prompt
    ...
    # event_line
    assert "A组织战略陪伴" in prompt
    assert "el_1" in prompt  # event_line id
    # documents
    assert "负责人甲秘书长见面纪要" in prompt
    # 维度指引
    for d in DIMENSIONS:
        assert d in prompt
...
def test_generator_uses_real_facts_when_ai_ready(db: Database) -> None:
    ...
    assert "负责人甲" in dims["people"]["narrative"]
