...
def test_event_log_insert_and_query(db: Database):
    ...
    db.conn.execute(
        ...
        (
            ...
            "fact_abc123",
            "client_xyz",
            json.dumps({"subject": "日慈", "attribute": "法人代表"}),
            "2026-05-22T10:00:00",
        ),
    )
    db.conn.commit()
...
def test_atomic_facts_can_insert_with_new_metadata(db: Database):
    ...
    db.conn.execute(
        """INSERT INTO clients(id, name, alias, domain, type, intro, stage, color,
           ...
           VALUES('c1','日慈','日慈','项目','项目','','active','#5B7BFE',?,?)""",
    ...
    db.conn.execute(
        ...
        INSERT INTO atomic_facts (
            ...
            'fact_1', 'c1', '张真', '角色', '法人代表', '法人代表',
            0.95, 'active', '2026-05-22T10:00:00', '2026-05-22T10:00:00',
            'client_verbal_meeting', 'fact', 'human', 'user_guyuanyuan',
