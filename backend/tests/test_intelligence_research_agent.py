...
class _ReadyAi:
    ...
    def generate_general_fallback(self, *_args, **_kwargs):
        return AiStructuredResponse(
            ...
            analysis="可复用事实：\n- 广东省A组织公益基金会公开材料提到心灵魔法学院项目，说明该项目面向儿童青少年心理健康服务。\n- 广东省A组织公益基金会材料提到青年支持计划关注困境儿童心理健康，可作为项目方向线索。",
            actions="",
            timeline="",
        )
...
class _ReadyTimelyAi:
    ...
    def generate_general_fallback(self, *_args, **_kwargs):
        return AiStructuredResponse(
            ...
            judgment="该机会与A组织关注的儿童心理健康服务方向相关，可能影响其项目合作、资源争取和材料准备节奏。",
            analysis="如果A组织具备对应服务基础，这类窗口可转化为合作或申报机会；若资格、地域或服务对象不匹配，则只应纳入机会观察清单。",
...
def test_research_profile_card_requires_focus_evidence_and_reuses_body_quotes(tmp_path: Path, monkeypatch) -> None:
    ...
    _seed_client(db, client_id="client_rici", name="广东省A组织公益基金会")
    _insert_focus(db, client_id="client_rici", profile=["A组织官网介绍。心灵魔法学院、青年支持计划两大项目的案例和数据。"])
    intent = _intent("client_rici", "广东省A组织公益基金会 心灵魔法学院 项目介绍", content_kind="profile_completion")
    ...
    monkeypatch.setattr(
        ...
        lambda _url: (
            "fetched",
            "广东省A组织公益基金会官网介绍，心灵魔法学院项目面向儿童青少年心理健康服务，提供课程、活动和家庭支持。"
            "广东省A组织公益基金会在青年支持计划中持续关注困境儿童心理健康，公开材料提到项目服务对象、执行方法和案例。",
            "",
        ),
    )
    ...
    def fetcher(_query: str, source_config) -> list[CandidateHit]:
        return [
            CandidateHit(
                title="广东省A组织公益基金会心灵魔法学院项目介绍",
                url="https://www.rici.org.cn/projects/magic",
                snippet="广东省A组织公益基金会官网公开心灵魔法学院项目介绍。",
    ...
    result = run_intelligence_candidate_refresh(
        ...
        data_dir=tmp_path,
        ai_service=_ReadyAi(),
        scope=_scope("client_rici", "广东省A组织公益基金会"),
        intents=[intent],
        max_fetch_jobs=1,
        hit_fetcher=fetcher,
...
def test_research_profile_focus_is_priority_not_hard_gate_for_basic_gaps(tmp_path: Path, monkeypatch) -> None:
    ...
    _seed_client(db, client_id="client_rici", name="广东省A组织公益基金会")
    _insert_focus(db, client_id="client_rici", profile=["心灵魔法学院、青年支持计划两大项目的案例和数据。"])
    intent = _intent("client_rici", "广东省A组织公益基金会 登记信息", content_kind="profile_completion")
    _insert_intent(db, intent)
    monkeypatch.setattr(
        supply,
        "_fetch_page_text",
        lambda _url: (
            "fetched",
            "广东省A组织公益基金会登记信息显示，该机构属于社会组织，页面包含统一社会信用代码、登记机关和业务范围。",
            "",
        ),
    )
    ...
    result = run_intelligence_candidate_refresh(
        ...
        hit_fetcher=lambda _query, source_config: [
            CandidateHit(
                title="广东省A组织公益基金会社会组织登记信息",
                url="https://gdnpo.gov.cn/org/rici",
                snippet="广东省A组织公益基金会登记信息。",
                source=source_config.source_name,
            )
        ],
        official_site_hit_fetcher=lambda _query, _config: [],
...
def test_research_user_supplied_url_becomes_trusted_official_source(tmp_path: Path) -> None:
    ...
    _seed_client(db, client_id="client_yiyu", name="示例工作台", domain="公益组织数字化")
    _insert_focus(db, client_id="client_yiyu", profile=["示例工作台/管理员甲的观点、文章、案例。官网（www.yiyu.love）的示例团队介绍。"])
    intent = _intent("client_yiyu", "示例工作台 管理员甲 观点 文章", content_kind="profile_completion", intent_id="intent_yiyu")

    run_intelligence_candidate_refresh(
        db,
        data_dir=tmp_path,
        ai_service=None,
        scope=_scope("client_yiyu", "示例工作台"),
        intents=[intent],
        max_fetch_jobs=1,
...
def test_research_list_page_drills_down_but_never_promotes_list_itself(tmp_path: Path, monkeypatch) -> None:
    ...
    _seed_client(db, client_id="client_rici", name="广东省A组织公益基金会")
    _insert_focus(db, client_id="client_rici", profile=["负责人甲接受采访的报道。"])
    intent = _intent("client_rici", "广东省A组织公益基金会 负责人甲 采访 报道", content_kind="profile_completion")
    _insert_intent(db, intent)
    monkeypatch.setattr(
        supply,
    ...
    monkeypatch.setattr(
        ...
        lambda _url: (
            "fetched",
            "广东省A组织公益基金会秘书长负责人甲接受采访时介绍，基金会关注儿童心理健康服务，并结合项目案例推动困境儿童支持。",
            "",
        ),
    )
    ...
    result = run_intelligence_candidate_refresh(
        ...
        hit_fetcher=lambda _query, source_config: [
            CandidateHit(
                title="广东省A组织公益基金会新闻列表",
                url="https://www.rici.org.cn/news?page=1",
                snippet="新闻列表，含负责人甲采访报道。",
                source=source_config.source_name,
            )
        ],
        official_site_hit_fetcher=lambda _query, _config: [],
