...
def test_long_focus_is_split_into_short_queries_and_site_queries(tmp_path: Path) -> None:
    ...
    long_focus = "示例工作台/管理员甲的观点、文章、案例，以及官网（www.yiyu.love）的示例团队介绍、公开服务对象和合作案例。"
    ...
    assert any("管理员甲" in query and "观点" in query for query in queries)
...
def test_basic_profile_gaps_are_searched_even_with_narrow_focus(tmp_path: Path) -> None:
    ...
    _seed_focus(db, client_id="client_rici", profile=["心灵魔法学院、青年支持计划、负责人甲采访。"])
...
def test_profile_search_relaxes_after_many_no_results(tmp_path: Path) -> None:
    ...
    for index in range(9):
        db.execute(
            ...
            """,
            (f"job_no_result_{index}", f"广东省A组织公益基金会 空搜 {index}", f"2026-05-14T10:0{index}:00"),
        )
...
def test_candidate_refresh_reports_search_surface_diagnostics(tmp_path: Path, monkeypatch) -> None:
    ...
    _seed_client(db, client_id="client_rici", name="广东省A组织公益基金会")
    intent = _intent("client_rici", "广东省A组织公益基金会 心灵魔法学院", content_kind="profile_completion")
    _insert_intent(db, intent)
    monkeypatch.setattr(
        "app.services.intelligence_candidate_supply._fetch_page_text",
        lambda _url: (
            "fetched",
            "广东省A组织公益基金会官网材料提到心灵魔法学院项目，项目面向儿童青少年心理健康服务，提供课程和家庭支持。",
            "",
        ),
    )
