...
def test_build_agent_worklog_response_reads_db_and_thread_sync(tmp_path: Path):
    ...
    db.execute(
        ...
        ("log_1", "成员甲", "review.create", "weekly_review", "review_1", "{}", "2026-03-15T10:00:00"),
    )
    db.execute(
    ...
    db.execute(
        ...
        ("task_1", "推进战略问答整理", "", "doing", "high", "list-0", "成员甲", "周五", "manual", None, "[]", "[]", "2026-03-15T09:00:00", "2026-03-15T09:30:00"),
    )
    ...
    assert {item.agentName for item in response.worklogs} == {"成员甲", "大周", "佳乐"}
    ...
    assert any(digest.agentName == "成员甲" and digest.focusItems for digest in response.weeklyDigests)
    assert any(plan.agentName == "成员甲" and plan.planItems for plan in response.weeklyPlans)
...
def test_monthly_worklog_response_uses_full_week_digest_range(tmp_path: Path):
    ...
    db.execute(
        "INSERT INTO activity_logs(id, actor_name, action, entity_type, entity_id, detail_json, created_at) VALUES(?, ?, ?, ?, ?, ?, ?)",
        ("log_1", "成员甲", "task.update", "task", "task_1", "{}", "2026-03-31T09:00:00"),
    )
    db.execute(
        "INSERT INTO activity_logs(id, actor_name, action, entity_type, entity_id, detail_json, created_at) VALUES(?, ?, ?, ?, ?, ?, ?)",
        ("log_2", "成员甲", "review.create", "weekly_review", "review_1", "{}", "2026-04-01T10:00:00"),
    )
    thread_sync_path = tmp_path / "thread-sync.md"
    ...
    march_digest = next((item for item in monthly_response.weeklyDigests if item.agentName == "成员甲" and item.weekLabel == "2026-W14"), None)
    ...
    assert weekly_digests[0].agentName == "成员甲"
    ...
    assert weekly_plans[0].agentName == "成员甲"
...
def test_weekly_plans_apply_manual_override(tmp_path: Path):
    ...
    db.execute(
        "INSERT INTO activity_logs(id, actor_name, action, entity_type, entity_id, detail_json, created_at) VALUES(?, ?, ?, ?, ?, ?, ?)",
        ("log_1", "成员甲", "task.update", "task", "task_1", "{}", "2026-03-17T09:00:00"),
    )
    db.execute(
        """
    ...
    qinghua_plan = next(item for item in weekly_plans if item.agentName == "成员甲")
...
def test_build_agent_weekly_review_items_turns_robot_work_into_review_samples(tmp_path: Path):
    ...
    qinghua_item = next(item for item in items if item.taskSnapshot.ownerName == "成员甲")
