...
3. 顾源源"信息商" 洞察: conflict vs supersedes vs complement 判断
...
# ════════════════════════════════════════════════════════════════
# 1. 渠道驱动 content_role 兜底 (顾源源 5/22 规则)
# ════════════════════════════════════════════════════════════════
...
def test_workbench_file_contract_metadata():
    ...
    assert md.actor_id == "user_guyuanyuan"
...
def test_supersede_keyword_重签_detected():
    """顾源源场景: '合同金额 300 万要改成 800 万重签' → supersedes"""
...
def test_ingest_writes_ai_episode_log_when_session_given(db: Database):
    """如果带 ai_session_id, 顺手写 ai_episode_log (顾源源 5/22 决策: v2.2 阶段就写)"""
    pipeline = IngestPipeline(db)
    md = metadata_for_workbench_file(
...
def test_ingest_skips_duplicate_value(db: Database):
    ...
    req1 = IngestRequest(
        ...
        subject_text="日慈", attribute="法人", value_text="张真",
        metadata=md,
    )
...
def test_ingest_supersedes_old_fact_with_update_keyword(db: Database):
    """★ 顾源源场景核心: 用户说'改成 800 万要重签' → 旧事实 superseded, 新事实 current"""
    pipeline = IngestPipeline(db)
    # 先写一个旧合同金额
    md_old = metadata_for_workbench_file(file_doc_type="contract", actor_id="u1")
    req_old = IngestRequest(
        path="workbench_file", client_id="c_test",
        subject_text="日慈", attribute="合同金额", value_text="300 万",
        metadata=md_old,
    )
    ...
    # 用户在手机 AI 聊天里说要重签
    md_new = metadata_for_mobile_ai_chat(user_id="user_guyuanyuan", is_user_subjective=False)
    req_new = IngestRequest(
        path="mobile_ai_chat", client_id="c_test",
        subject_text="日慈", attribute="合同金额",
...
def test_ingest_conflict_when_no_update_semantic(db: Database):
    ...
    pipeline.ingest(IngestRequest(
        path="workbench_file", client_id="c_test",
        subject_text="日慈", attribute="员工数", value_text="50 人",
        metadata=md1,
    ))
    ...
    r2 = pipeline.ingest(IngestRequest(
        ...
        subject_text="日慈", attribute="员工数", value_text="120 人",
        metadata=md2,
    ))
...
def test_log_ai_episode_writes_record(db: Database):
    ...
    log_ai_episode(
        ...
        action_summary="我无法分辨张真接的是法人还是理事长, 请补充",
        user_id="user_guyuanyuan",
        client_id="c_test",
    )
    db.conn.commit()
    ...
    assert row is not None
    assert row["action_type"] == "requested_human_help"  # 顾源源洞察 4: AI 反向给人类提请求
