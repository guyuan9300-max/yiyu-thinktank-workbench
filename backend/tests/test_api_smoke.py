...
def test_health_and_structural_defaults(tmp_path: Path):
    ...
    payload = health.json()
    assert payload["appName"] == "示例工作台自用平台"
...
def test_strategic_meeting_pack_apply_updates_cockpit_snapshot(tmp_path: Path, monkeypatch):
    ...
    ingest_response = client.post(
        ...
        json={
            ...
            "notes": "决定先把客户盘点会固定下来。成员甲负责下周推进。当前还缺关键资料待补。",
        },
    )
...
def test_feishu_user_binding_callback_persists_current_user(tmp_path: Path, monkeypatch):
    ...
    session_user = seed_session_user(client, user_id="user_feishu", email="admin-demo@example.org", full_name="管理员甲")
    seed_cloud_token(client)
    ...
    assert binding_payload["email"] == "admin-demo@example.org"
    assert binding_payload["name"] == "管理员甲"
...
def test_feishu_user_binding_start_uses_configured_public_callback(tmp_path: Path, monkeypatch):
    ...
    session_user = seed_session_user(client, user_id="user_feishu_callback", email="admin-demo@example.org", full_name="管理员甲")
    seed_cloud_token(client)
    stub_cloud_auth_me(monkeypatch, session_user)
...
def test_feishu_meeting_launch_prefers_bound_user_over_global_receiver(tmp_path: Path, monkeypatch):
    ...
    session_user = seed_session_user(client, user_id="user_feishu_bound", email="admin-demo@example.org", full_name="管理员甲")
    ...
    assert launch_payload["deliveryTarget"] == "管理员甲"
...
def test_demo_data_can_be_loaded_on_demand(tmp_path: Path):
    ...
    assert {item["name"] for item in clients.json()} == {"C项目", "星辰科技"}
...
def test_client_meeting_publish_writes_task(tmp_path: Path):
    ...
    ingest = client.post(
        f"/api/v1/clients/{client_id}/meetings/{meeting_id}/ingest",
        json={"transcriptText": "决定本周由成员甲负责补齐方案，跟进捐赠人反馈。", "notes": "待确认时间点。"},
    )
...
情报判断：
这条线索与A组织的儿童青少年心理健康方向有关，但正式申报主体、联合申报规则和材料清单仍需核验。
...
def test_intelligence_baseline_schema_api_and_legacy_topics_compatibility(tmp_path: Path):
    ...
    radar = client.post(
        ...
        json={
            ...
            "targetObject": {"type": "org", "id": "org_1", "label": "示例工作台"},
...
def test_intelligence_profile_outputs_work_context_and_background_enrichment_is_not_topic(tmp_path: Path, monkeypatch):
    ...
    db.execute(
        ...
        INSERT INTO memory_facts(
            id, scope_type, scope_id, fact_key, fact_value, source_type, source_id,
            confidence, freshness, created_at, updated_at
        ) VALUES(?, 'client', ?, '客户公开身份', '广东省A组织公益基金会是关注儿童青少年心理健康、社区服务和公益资助合作的基金会。', 'manual', 'test', 0.86, 0.9, ?, ?)
        """,
    ...
    def fake_fetch(*args, **kwargs):
        return [
            TopicSearchHit(
                title="广东省A组织公益基金会机构简介",
                summary="广东省A组织公益基金会机构介绍、业务范围、公开联系方式和项目背景资料。",
                source="A组织官网",
...
def test_intelligence_capture_filters_weak_signals_and_keeps_selected_advisor_item(tmp_path: Path, monkeypatch):
    ...
    db.execute(
        ...
        INSERT INTO memory_facts(
            id, scope_type, scope_id, fact_key, fact_value, source_type, source_id,
            confidence, freshness, created_at, updated_at
        ) VALUES(?, 'client', ?, '情报优先方向', 'A组织当前优先寻找广东地区儿童青少年心理健康服务相关的资助机会、合作方和政策窗口。', 'user_context', 'test', 0.9, 0.9, ?, ?)
        """,
...
def test_topic_candidate_insight_uses_advisor_context_and_refreshes_when_facts_change(tmp_path: Path, monkeypatch):
    ...
    db.execute(
        ...
        INSERT INTO memory_facts(
            id, scope_type, scope_id, fact_key, fact_value, source_type, source_id,
            confidence, freshness, created_at, updated_at
        ) VALUES(?, 'client', ?, '近期情报需求', 'A组织近期优先判断广东地区儿童青少年心理健康服务的资助机会，尤其关注能否联合学校、社区或本地社会组织申报。', 'user_context', 'test', 0.92, 0.9, ?, ?)
        """,
    ...
    def fake_insight_builder(**kwargs):
        ...
        return {
            ...
            "recommendationReasons": [str(item) for item in facts[:3]] or ["命中A组织近期资助机会判断需求"],
            "practicalUses": ["先核验申报主体、截止时间、地域限制和联合申报要求。", "准备A组织儿童青少年心理健康服务案例、一页纸项目说明和预算框架。", "若要求南沙本地执行主体，优先寻找学校、社区或本地社会组织作为合作方；若不接受联合申报则止损。"],
            "editorialNote": "情报研判：这条线索的价值不在于“又有一个公益创投”，而在于它同时命中A组织当前的儿童青少年心理健康服务方向、广东地域和资助合作窗口。结合近期任务里“先核验主体、合作方式和材料清单”的判断，它更适合作为快速初筛机会，而不是直接进入完整申报。真正要先确认的是南沙本地执行主体、联合申报是否可行、截止时间和预算口径；这些条件成立时，A组织可以用现有服务案例和教师/青少年心理支持清单转成一页纸方案。",
            "discussionPrompts": ["这条机会适合A组织独立申报还是联合申报？", "如果转任务，负责人先核验哪三个条件？"],
            "_generationSource": "test",
        }
    ...
    assert "整理A组织可申报项目清单" in context_text
    ...
    db.execute(
        ...
        INSERT INTO memory_facts(
            id, scope_type, scope_id, fact_key, fact_value, source_type, source_id,
            confidence, freshness, created_at, updated_at
        ) VALUES(?, 'client', ?, '申报预算准备', 'A组织目前需要补一版青少年心理健康服务预算框架，才能快速判断50万元以内项目是否值得申报。', 'user_context', 'test', 0.9, 0.9, ?, ?)
        """,
...
def test_workspace_import_auto_generates_client_dna_candidates(tmp_path: Path, monkeypatch):
    ...
    (source / "项目介绍.md").write_text(
        "# 项目介绍\n示例工作台是一家长期服务公益机构的战略陪伴团队，当前项目聚焦任务协同、会议复盘与资料底盘建设。\n"
        "团队成员包括管理员甲、成员甲和罗茜茜，当前重点是把组织介绍、项目介绍和市场背景统一成系统上下文。",
        encoding="utf-8",
    )
    ...
    def fake_generate_structured(prompt: str, system_instruction: str, context_summary: str) -> AiStructuredResponse:
        if "组织介绍" in prompt:
            return AiStructuredResponse(
                content="## 1. 组织定位\n示例工作台是一家面向公益机构的战略陪伴与知识底盘建设团队。\n\n## 2. 工作方式\n强调陪伴式共创、任务协同和资料沉淀。",
                judgment="示例工作台聚焦公益行业的战略陪伴与知识底盘建设，强调陪伴式共创和长期沉淀。",
                analysis="仍缺组织发展历史\n仍缺核心使命的更完整表述",
                actions="项目介绍.md",
...
def test_identity_role_query_requires_explicit_role_evidence(tmp_path: Path, monkeypatch):
    client = make_client(tmp_path)
    client_id = create_test_client_record(client, name="B客户")
    transcript = tmp_path / "workshop.md"
    transcript.write_text(
        "# 战略工作坊实录\n管理员甲在本次工作坊中主要负责提问、梳理业务逻辑和推进后续访谈，并未担任 B客户 内部角色。",
        encoding="utf-8",
    )
    imported = client.post(
        "/api/v1/imports",
    ...
    response = client.post(
        f"/api/v1/clients/{client_id}/workspace/chat",
        json={"prompt": "B客户 创始人是什么样的人，分析一下"},
    )
    assert response.status_code == 200
    payload = response.json()
...
def test_vectorize_answer_creates_memory_doc_and_export_answer_writes_docx(tmp_path: Path):
    ...
    file_path.write_text(
        "# C项目机构介绍\nC项目聚焦山区儿童教育，当前重点在捐赠人沟通和项目里程碑梳理。",
        encoding="utf-8",
    )
    imported = client.post(
        "/api/v1/imports",
...
def test_chat_falls_back_to_local_retrieval_summary_when_llm_generation_times_out(tmp_path: Path, monkeypatch):
    ...
    file_path = tmp_path / "org-intro.md"
    file_path.write_text(
        "# C项目机构介绍\nC项目聚焦山区儿童教育，已经形成机构介绍、项目推进和捐赠人沟通三条主线。",
        encoding="utf-8",
    )
    imported = client.post(
        "/api/v1/imports",
    ...
    answer = client.post(
        f"/api/v1/clients/{client_id}/workspace/chat",
        json={"prompt": "C项目是一家什么样的机构？请简要介绍。"},
    )
    assert answer.status_code == 200
    payload = answer.json()
...
def test_intro_fallback_filters_ppt_noise_and_keeps_client_materials_with_service_provider_mentions(tmp_path: Path, monkeypatch):
    client = make_client(tmp_path)
    client_id = create_test_client_record(client, name="A组织")
    meeting_path = tmp_path / "teacher-note.md"
    meeting_path.write_text(
        "# A组织教师赋能会议纪要\nA组织当前围绕教师赋能推进带领者培养、社群运营和数字化协作，资料中也提到示例工作台支持后续协同，但主体仍是客户项目推进与阶段判断。",
        encoding="utf-8",
    )
    ppt_path = tmp_path / "slide-noise.pptx"
    ppt_path.write_text(
        "单击此处编辑母版文本样式 演示文稿标题 演示文稿副标题 作者和日期 开启A组织的战略第二曲线",
        encoding="utf-8",
    )
    imported = client.post(
        "/api/v1/imports",
    ...
    assert "示例工作台支持" in payload["content"]
...
def test_organization_dna_upload_replace_and_settings_roundtrip(tmp_path: Path):
    ...
    assert "示例工作台" in payload["normalizedText"]

    replaced = client.post(
        "/api/v1/settings/org-dna/organization_intro",
        json={
          "markdownContent": "# 组织介绍\n示例工作台专注于公益咨询、任务协同与知识工作台。",
...
def test_tasks_consume_project_dna_and_module_flow_context(tmp_path: Path):
    ...
    uploads = {
        ...
        "team_intro": "# 团队介绍\n项目由管理员甲总体负责，罗茜茜负责客户协同，成员甲负责策略研判。",
...
def test_notebook_sanitizes_prompt_and_executive_summary_pollution(tmp_path: Path):
    ...
    db.execute(
        ...
        """,
        (
            '{"prompt":"你将作为深度研究+写作综合体","title":"A组织机构DNA"}',
            "2026-03-23T10:00:00",
            client_id,
        ),
    )
...
def test_task_memory_enrichment_matches_notebook_references(tmp_path: Path):
    ...
    dna_response = client.post(
        f"/api/v1/clients/{client_id}/dna-documents/organization_intro",
        json={
            "markdownContent": "# 组织介绍\n\n敦和基金会当前由林红负责合作拓展，正在与示例团队讨论联合研究和后续方案边界。",
            "fileName": "组织介绍.md",
        },
    )
    assert dna_response.status_code == 200, dna_response.text
...
def test_event_line_context_bundle_returns_memory_tasks_and_evidence(tmp_path: Path):
    client = make_client(tmp_path)
    client_id = create_test_client_record(client, "A组织")
    ...
    updated_event_line = client.patch(
        f"/api/v1/event-lines/{event_line_id}",
        json={
            "currentBlocker": "负责人甲还没有把这次系统演示和内部协同目标正式对齐。",
    ...
    created_task = client.post(
        "/api/v1/tasks",
        json={
            "title": "给A组织负责人甲看示例团队系统",
            "desc": "通过现场演示让负责人甲判断系统和当前合作目标的关系，并收集会后动作。",
            "priority": "high",
            "listId": "list-0",
            "clientId": client_id,
            "eventLineId": event_line_id,
    ...
    clarification = client.post(
        ...
        json={
            ...
            "scopeId": client_id,
            "slotKey": "partner_context",
            "question": "负责人甲在A组织当前负责什么？",
        },
    )
    assert clarification.status_code == 200, clarification.text
    answered = client.post(
        f"/api/v1/clarifications/{clarification.json()['id']}/answer",
        json={"answer": "负责人甲目前负责A组织和示例团队的系统演示与下一步合作判断。"},
    )
    assert answered.status_code == 200, answered.text
    ...
    upload_response = client.post(
        ...
        files={
            "file": (
                "A组织系统演示反馈.md",
                "# 演示反馈\n\n负责人甲重点关注系统与组织现有协同流程的贴合度。".encode("utf-8"),
                "text/markdown",
            )
        },
    )
    ...
    assert payload["lineName"] == "A组织系统演示推进线"
    assert payload["currentBlocker"] == "负责人甲还没有把这次系统演示和内部协同目标正式对齐。"
    ...
    assert any("负责人甲" in item["summary"] for item in payload["clarificationFacts"])
...
def test_task_context_preview_returns_bundle_and_judgment(tmp_path: Path):
    client = make_client(tmp_path)
    client_id = create_test_client_record(client, "B客户")
    ...
    created_event_line = client.post(
        ...
        json={
            ...
            "status": "active",
            "stage": "战略陪伴中",
            "summary": "围绕 B客户 的系统演示、合作判断和下一步行动持续推进。",
    ...
    assert payload["contextBundle"]["lineName"] == "B客户系统演示推进线"
    ...
    assert "B客户" in payload["judgment"]["whatHappened"]
...
def test_weekly_review_analysis_ignores_polluted_event_line_background(tmp_path: Path):
    ...
    db.execute(
        ...
        INSERT INTO tasks(
            id, title, description, status, priority, list_id, client_id, event_line_id, project_module_id, project_flow_id,
            ddl, due_date, duration_minutes, owner_name, source_type, source_id, tags_json, tag_ids_json, created_at, updated_at
        ) VALUES(?, ?, ?, 'doing', 'high', 'list-0', ?, ?, NULL, NULL, '本周', NULL, 60, '管理员甲', 'manual', NULL, '[]', '[]', ?, ?)
        """,
        (
            "task_dirty_review",
            "向联系人丙老师介绍数字化系统",
            ...
            json.dumps(
                {
                    ...
                    "eventLineContext": {
                        ...
                        "name": "向光奖联系人丙老师介绍数字化系统",
                        "summary": "执行摘要：示例工作台是专注于“可落地增长咨询”的战略陪伴者。",
                        "intent": '{"prompt":"你将作为深度研究 + 机构与项目运营写作综合体"}',
                        "currentBlocker": "当前没有特别突出的阻塞，但仍需盯住推进收束。",
                        "recentDecision": "最近线索：正文 示例团队2026一季度计划 传统战略咨询这个品类正在收缩。",
                        "nextStep": "建议先明确这一阶段的核心事项。",
                    },
                    "projectContext": {
                        "currentFocus": "执行摘要：示例工作台是专注于“可落地增长咨询”的战略陪伴者。"
                    },
                },
                ensure_ascii=False,
            ),
    ...
    assert "向联系人丙老师介绍数字化系统" in summary["whatHappenedThisWeek"]
...
def test_reviews_route_accepts_notebook_and_event_line_memory_evidence_refs(tmp_path: Path):
    ...
    db.execute(
        ...
        (
            ...
            client_id,
            "该机构正在推进数字化合作验证。",
            "当前与示例团队处于方案共创与会谈推进阶段。",
            "推进中",
            json.dumps(["负责人甲"], ensure_ascii=False),
            json.dumps(["示例团队系统"], ensure_ascii=False),
    ...
    db.execute(
        ...
        INSERT INTO tasks(
            id, title, description, status, priority, list_id, client_id, event_line_id, project_module_id, project_flow_id,
            ddl, due_date, duration_minutes, owner_name, source_type, source_id, tags_json, tag_ids_json, created_at, updated_at
        ) VALUES(?, ?, ?, 'doing', 'high', 'list-0', ?, ?, NULL, NULL, '本周', NULL, 60, '管理员甲', 'meeting', 'meeting_demo_1', '[]', '[]', ?, ?)
        """,
        (
            "task_review_sources",
            "给A组织负责人甲看示例团队系统",
