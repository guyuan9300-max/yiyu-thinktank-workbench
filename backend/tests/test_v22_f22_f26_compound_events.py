...
def test_org_events_insert_personnel_change(db: Database):
    """实战场景: 老师乙离职 - 人员变动 + high severity + negative impact"""
    db.conn.execute(
        """
        ...
        (
            ...
            "personnel_change",
            "老师乙离职",
            "5 月初老师乙因健康原因离职, 章方同时辞职, 青年支持计划主理交接给王强",
...
def test_event_line_state_changes_can_record_status_transition(db: Database):
    ...
    db.conn.execute(
        ...
        (
            ...
            "human", "user_example_user", "medium",
        ),
    )
    db.conn.commit()
    # 按时间顺序读, 就是这条主线的完整故事
...
def test_event_line_state_changes_can_be_reversed(db: Database):
    ...
    db.conn.execute(
        ...
        ("2026-05-22T11:00:00", "老师乙离职跟这条主线没关系, AI 推理错了", change_id),
    )
    db.conn.commit()
    row = db.fetchone(
...
# ════════════════════════════════════════════════════════════════
# 跨表协作场景: AI 端到端理解"A组织品牌升级主线"
# ════════════════════════════════════════════════════════════════


def test_end_to_end_brand_pivot_story(db: Database):
    """完整故事: 老师乙离职 → 主线阻塞 → 决策接手 → 主线恢复

    管理员甲 5/22 原话: 让 AI 能讲清楚"主线怎么走过来的"
    """
    # 1. 写组织事件: 老师乙离职
    db.conn.execute(
        """INSERT INTO org_events (
    ...
    assert len(rows) == 2
    # 第 1 步: blocked, 触发源是老师乙离职事件
    assert rows[0]["to_status"] == "blocked"
    assert rows[0]["trigger_summary"] == "老师乙离职"
