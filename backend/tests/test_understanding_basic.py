...
def _make_org_dna() -> list[OrganizationDnaModuleRecord]:
    return [
        OrganizationDnaModuleRecord(
            ...
            title="组织介绍",
            markdownContent="示例工作台是一家专注于公益行业的咨询公司",
            normalizedText="示例工作台是一家专注于公益行业的咨询公司，为基金会和公益组织提供战略咨询、数字化转型和研究服务。",
            summary="示例工作台是公益行业咨询公司，提供战略咨询和数字化转型服务。",
        ),
    ]
...
class _FakeBadBriefAi:
    def _qwen_generate(self, **kwargs):
        return {
            "humanBrief": "这是一条 todo 状态任务，与客户 B客户 相关。",
            "whatIsThis": "旧模板",
            "whyItMatters": "旧模板",
            "progressNow": "旧模板",
            "unknowns": "旧模板",
...
class TestBasicModeMinimalInput:
    ...
    def test_with_client_background(self):
        pc = TaskProjectContextRecord(
            ...
            clientName="B客户",
            backgroundSummary="B客户是公益行业的重要枢纽组织",
    ...
    def test_with_review_note(self):
        entry = _make_entry(note="本周和联系人甲老师确认了工作坊方向，下周准备正式提案。")
        result = build_understanding_basic(ai=None, task_entry=entry, org_dna_modules=[])
        assert "复盘" in result.progressNow or "联系人甲" in result.progressNow or "说明" in result.progressNow
