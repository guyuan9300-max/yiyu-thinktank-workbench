...
def _seed_client(db: Database, *, client_id: str = "client_rici", name: str = "广东省A组织公益基金会") -> None:
    db.execute(
        """
...
def test_p4_official_site_discovery_creates_fixed_section_sources_and_filters_bad_domains(tmp_path: Path) -> None:
    ...
    def fake_fetcher(_query: str, _config) -> list[CandidateHit]:
        return [
            CandidateHit(
                title="广东省A组织公益基金会 官网 信息公开",
                url="https://www.rici.org.cn/about",
                snippet="广东省A组织公益基金会官方网站，公开项目介绍和信息公开。",
                source="广东省A组织公益基金会",
            ),
            CandidateHit(
...
def test_p4_route_matching_keeps_web_scout_before_specific_sources(tmp_path: Path) -> None:
    ...
    grant_task = supply._selected_fetch_tasks(
        db,
        [_intent("广东省A组织公益基金会 公益创投 申报 征集", intent_id="intent_grant")],
        configs,
        max_fetch_jobs=2,
    )
    procurement_task = supply._selected_fetch_tasks(
        db,
        [_intent("广东省A组织公益基金会 政府购买服务 招标 采购", intent_id="intent_procurement")],
        configs,
        max_fetch_jobs=2,
    )
    risk_task = supply._selected_fetch_tasks(
        db,
        [_intent("广东省A组织公益基金会 公开募捐 监管 风险提示", intent_id="intent_risk")],
        configs,
        max_fetch_jobs=2,
    )
...
def test_p4_body_quality_rejection_is_recorded_before_profile_card(tmp_path: Path, monkeypatch) -> None:
    ...
    _seed_client(db)
    scope = _scope()
    intent = _intent("广东省A组织公益基金会 信息公开 年报", content_kind="profile_completion", intent_id="intent_profile")
    ...
    def fake_fetcher(_query: str, source_config) -> list[CandidateHit]:
        return [
            CandidateHit(
                title="广东省A组织公益基金会 信息公开 年报",
                url="https://www.rici.org.cn/report",
                snippet="广东省A组织公益基金会信息公开和年度报告。",
...
def test_p4_source_health_retry_and_notice_duplicate_merge(tmp_path: Path) -> None:
    ...
    _seed_client(db)
    scope = _scope()
    intent = _intent("广东省A组织公益基金会 公益创投 申报 征集", intent_id="intent_duplicate")
    _insert_intent(db, intent)
    ...
    failing_intent = _intent("广东省A组织公益基金会 公开募捐 监管 风险提示", intent_id="intent_fail")
