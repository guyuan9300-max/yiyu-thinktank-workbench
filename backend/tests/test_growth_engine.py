...
def seed_client_and_event_line(db: Database) -> None:
    db.execute(
        ...
        ("client_1", "A组织", "A组织", "philanthropy", "client", "公益客户", "active", "2026-03-10T09:00:00", "2026-03-10T09:00:00"),
    )
    db.execute(
        """
...
def test_meeting_candidate_generates_contextual_growth_entries(tmp_path: Path):
    ...
    assert any(item.label == "A组织" for item in overview.projectGrowthHighlights)
...
def test_handbook_reuse_records_hard_context_evidence(tmp_path: Path):
    ...
    assert detail["sourceLabel"] == "A组织季度对齐会筹备"
    ...
    assert signal_context["sourceTitle"] == "A组织季度复盘会"
    assert signal_context["sourceLabel"] == "A组织季度对齐会筹备"
