...
def test_extract_cooperates_with() -> None:
    rels = extract_by_rules("A组织科技和阿里集团合作推出新产品。")
    assert _has(rels, "A组织科技", "cooperates_with", "阿里集团")


@pytest.mark.unit
def test_extract_competes_with_via_compound_term() -> None:
    rels = extract_by_rules("A组织科技的主要竞品是字节科技。")
    assert _has(rels, "A组织科技", "competes_with", "字节科技")


@pytest.mark.unit
def test_extract_invests_in() -> None:
    rels = extract_by_rules("阿里集团投资了A组织科技。")
    assert _has(rels, "阿里集团", "invests_in", "A组织科技")


@pytest.mark.unit
def test_extract_acquires() -> None:
    rels = extract_by_rules("百度收购了A组织科技。")
    assert _has(rels, "百度", "acquires", "A组织科技")
...
def test_extract_located_in() -> None:
    rels = extract_by_rules("A组织科技位于北京中关村。")
    assert _has(rels, "A组织科技", "located_in", "北京中关村")
...
def test_extract_dedupes_same_triple_in_one_chunk() -> None:
    ...
    text = "A组织科技和阿里集团合作。后来A组织科技和阿里集团合作的项目扩大了。"
    rels = extract_by_rules(text)
    matches = [r for r in rels if r.subject_text == "A组织科技" and r.object_text == "阿里集团"]
