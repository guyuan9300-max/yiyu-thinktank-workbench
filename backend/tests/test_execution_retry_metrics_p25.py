...
def create_client_record(client: TestClient, name: str = "execution-retry-metrics-p25") -> str:
    response = client.post(
        ...
        json={
            ...
            "domain": "公益",
            "type": "战略陪伴",
            "intro": "execution retry metrics p25",
            "stage": "推进中",
        },
    )
...
def test_execution_retry_metrics_api_p25(tmp_path: Path):
    client = make_client(tmp_path)
    client_id = create_client_record(client)
    db = client.app.state.app_state.db
    now = datetime.now().replace(microsecond=0).isoformat()

    insert_proposal(client, proposal_id="proposal_retry_metrics_1", client_id=client_id)
    insert_proposal(client, proposal_id="proposal_retry_metrics_2", client_id=client_id)
    ...
    db.execute(
        ...
        VALUES('ticket_retry_metrics_1', 'proposal_retry_metrics_1', ?, 'task_prep', 'failed', '{}', '{}',
               'idem_1', 3, 3, 'task_create_failed', ?, 'task_create_failed', NULL, ?, ?)
        """,
        (client_id, now, now, now),
    )
    db.execute(
        """
        INSERT INTO execution_tickets(
        ...
        VALUES('ticket_retry_metrics_2', 'proposal_retry_metrics_2', ?, 'meeting_followup', 'failed', '{}', '{}',
    ...
    db.execute(
        ...
        VALUES('log_retry_metrics_1', 'ticket_retry_metrics_1', 'execute_action', 'failed', 'boom', '{}', ?)
        """,
        (now,),
    )
    ...
    assert payload.get("totalTickets", 0) >= 2
    assert payload.get("failedTickets", 0) >= 2
    assert payload.get("retriedTickets", 0) >= 2
