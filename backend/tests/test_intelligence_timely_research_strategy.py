...
class _ReadyTimelyAi:
    ...
    def generate_general_fallback(self, *_args, **_kwargs):
        return AiStructuredResponse(
            ...
            judgment="这条机会与广东省A组织公益基金会关注的儿童心理健康服务相关，可能影响其合作机会、资源争取和项目材料准备。",
            analysis="如果A组织具备对应服务基础，可作为合作或申报窗口；若地域、资格或服务对象不匹配，则应纳入机会观察清单而非立即执行。",
...
def test_timely_strategy_reads_profile_cards_and_generates_strategy_queries(tmp_path: Path) -> None:
    ...
    _insert_focus(db, client_id="client_rici", timely=["儿童青少年心理健康、心灵魔法学院、青年支持计划相关的资助、监管和合作动态。"])
    _insert_profile_card(
        db,
        client_id="client_rici",
        title="A组织心灵魔法学院资料卡",
        summary="A组织通过心灵魔法学院和青年支持计划服务儿童青少年心理健康，重点关注困境儿童、课程和平台建设。",
        tags=["项目介绍", "执行方法"],
    )
    ...
    assert "心灵魔法学院" in text or "青年支持计划" in text
...
def test_stale_source_with_future_effective_window_can_be_promoted(tmp_path: Path, monkeypatch) -> None:
    ...
    _insert_profile_card(
        ...
        client_id="client_rici",
        title="A组织儿童心理健康项目资料",
        summary="A组织关注儿童青少年心理健康服务和心理平台建设。",
    )
...
def test_timely_focus_does_not_mix_profile_focus_or_official_url(tmp_path: Path) -> None:
    ...
    _insert_focus(
        db,
        client_id="client_focus_split",
        profile=["官网 www.yiyu.love、管理员甲文章、官网案例和服务方案。"],
    ...
    _insert_profile_card(
        ...
        client_id="client_focus_split",
        title="示例工作台服务资料",
        summary="示例工作台关注公益组织数字化、AI 公益、服务方案和案例。",
        tags=["项目介绍", "执行方法"],
    )
...
def test_timely_external_signal_filters_current_official_domain_and_promotes_inspiration(tmp_path: Path, monkeypatch) -> None:
    ...
    _insert_focus(
        db,
        client_id="client_yiyu",
        profile=["官网 www.yiyu.love、管理员甲文章和案例。"],
        timely=["AI 公益、公益组织数字化、行业活动案例、资助合作动态。"],
    )
    _insert_profile_card(
        db,
        client_id="client_yiyu",
        title="示例工作台资料",
        summary="示例工作台服务公益组织数字化和 AI 公益，关注服务方案、行业案例和合作资源。",
        tags=["项目介绍", "执行方法"],
    )
    ...
    class _YiyuTimelyAi(_ReadyTimelyAi):
        def generate_general_fallback(self, *_args, **_kwargs):
            return AiStructuredResponse(
                ...
                judgment="这条外部机会虽未点名示例工作台，但与其公益组织数字化、AI 公益和服务方案案例方向高度相关，可作为合作资源和方案启发。",
                analysis="外部资金窗口可能通过社会组织数字化能力建设需求传导到示例工作台：客户需要应用场景、工具方案和案例材料，示例团队可研判是否作为合作方或服务支持方参与。",
...
def test_timely_scout_uses_web_search_and_compact_queries(tmp_path: Path) -> None:
    ...
    _insert_profile_card(
        ...
        client_id="client_rici",
        title="儿童心理健康服务资料",
        summary="A组织关注儿童青少年心理健康服务、心理平台建设和教师心理素养培训。",
    )
