...
def create_client_record(client: TestClient, name: str = "execution-ticket-retry-p24") -> str:
    response = client.post(
        ...
        json={
            ...
            "domain": "公益",
            "type": "战略陪伴",
            "intro": "execution ticket retry p24",
            "stage": "推进中",
        },
    )
...
def insert_approved_proposal(client: TestClient, *, proposal_id: str, client_id: str) -> None:
    ...
    client.app.state.app_state.db.execute(
        ...
        VALUES(?, ?, 'evidence_request', 'approved', 'medium', ?, ?, ?, ?, '[]', '[]', '{}', 'tester', 'tester', ?, NULL, NULL, ?, ?)
        """,
        (
            proposal_id,
            client_id,
...
def test_execution_ticket_retry_with_limit(tmp_path: Path):
    client = make_client(tmp_path)
    client_id = create_client_record(client)
    proposal_id = "proposal_exec_retry"
    ...
    # Simulate one failed execution so retry endpoint becomes available.
    db.execute(
        """
    ...
    assert retried_ticket.get("status") == "pending"
    assert int(retried_ticket.get("retryCount") or 0) == 1
    assert retried.json().get("proposal", {}).get("status") == "execution_pending"
