...
def test_calculate_priority_high_score():
    ...
    db.execute(
        ...
        "VALUES ('f_target', 'client_X', '日慈基金会', '法人代表', '张真', '张真', 0.4, 'supersedes', datetime('now'))",
    )
    ...
    db.execute(
        ...
        "VALUES ('f_target_old', 'client_X', '日慈基金会', '法人代表', '王晓燕', '王晓燕', 0.7, 'superseded', datetime('now'))",
    )

    # 加 active task 引用 subject → D3 行动依赖
    db.execute(
        "INSERT INTO tasks (id, client_id, title, description, status, due_date) "
        "VALUES ('t1', 'client_X', '日慈基金会方案', '法人确认', 'in_progress', datetime('now', '+5 days'))",
    )

    result = calculate_priority(
        db, "f_target", "client_X", "日慈基金会", "法人代表"
    )
...
def test_calculate_priority_low_score():
    ...
    db.execute(
        ...
        "VALUES ('f_stable', 'client_Y', '日慈基金会', '成立年份', '2013', 0.95, datetime('now'))",
    )

    result = calculate_priority(
        db, "f_stable", "client_Y", "日慈基金会", "成立年份"
    )
