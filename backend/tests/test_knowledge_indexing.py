...
def test_clean_title_for_search_strips_import_suffix_noise():
    assert clean_title_for_search("基业长青2022年财务报告_B客户_20260211.pdf") == "基业长青2022年财务报告"
    assert clean_title_for_search("B客户_项目价值分析表 2_B客户_20260211.pdf") == "B客户 项目价值分析表"


def test_build_catalog_search_text_prefers_summary_and_body_over_placeholder_noise():
    catalog = build_catalog_search_text(
        title="B客户文件+机构和业务简介-CFF2025年会手册_B客户_20260211.md",
        short_summary="B客户 的核心业务包括行业网络、数据资产和知识服务。",
        summary="这份材料系统介绍了机构定位、核心业务、行业角色与未来能力版图。",
        raw_text="B客户正在从传统信息平台转向公益慈善行业的可信基础设施，围绕网络、数据与知识形成三层底盘，并以行业服务、研究倡导、共创交付等方式形成长期复利。",
        keywords=["B客户", "机构介绍", "核心业务"],
        entities=["B客户", "行业协作平台"],
    ...
    assert "B客户 的核心业务包括行业网络、数据资产和知识服务" in catalog
...
def test_is_finance_query_detects_statement_style_questions():
    assert is_finance_query("分析B客户的财务状况")
    assert is_finance_query("有没有资产负债和现金流信息")
    assert not is_finance_query("介绍B客户的团队")


def test_is_finance_statement_query_detects_actual_statement_questions():
    assert is_finance_statement_query("分析B客户的财务状况")
...
def test_finance_document_score_adjustment_prefers_finance_reports_over_generic_intro_docs():
    finance_score = finance_document_score_adjustment(
        title="基业长青2023年财务报告_B客户_20260211.pdf",
        summary="包含资产总额、负债总额、收入总额与费用总额。",
        document_role="财务资料",
        folder_category="财务与筹款",
        path="/tmp/财务与筹款/基业长青2023年财务报告_B客户_20260211.pdf",
        statement_mode=True,
    )
    generic_score = finance_document_score_adjustment(
        title="B客户核心业务介绍 2_B客户_20260211.pdf",
        summary="介绍五个主要项目与平台化方向。",
        document_role="会议与访谈",
        folder_category="项目与业务",
        path="/tmp/项目与业务/B客户核心业务介绍_2_B客户_20260211.pdf",
        statement_mode=True,
    )
...
def test_finance_chunk_score_adjustment_prefers_numeric_finance_chunks():
    finance_chunk = finance_chunk_score_adjustment(
        title="3 基业长青2023年财务报告_B客户_20260211.pdf",
        excerpt="截止 2023 年 12 月 31 日，中心资产总额 751.44 万元，负债总额 42.62 万元，收入总额 880.51 万元。",
        section_label="第 1 页",
        path="/tmp/财务与筹款/3 基业长青2023年财务报告_B客户_20260211.pdf",
        statement_mode=True,
    )
