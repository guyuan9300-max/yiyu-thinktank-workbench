...
def test_user_ai_preferences_can_record(db: Database):
    ...
    db.conn.commit()
    row = db.fetchone(
        "SELECT * FROM user_ai_preferences WHERE user_id = 'user_example_user' AND preference_key = 'proposal_format'"
    )
    assert row is not None
...
def test_ai_feedback_signal_thumbs_down_with_correction(db: Database):
    ...
    db.conn.execute(
        ...
        """,
        ("ai_sess_001", "extracted_fact", "AI 说: 负责人甲接任理事长", "pending", "2026-05-22T11:00:00"),
    )
