...
def _seed_client(db: Database, *, client_id: str = "client_rici", name: str = "A组织") -> None:
    db.execute(
        """
...
def test_p3_negative_feedback_blocks_card_promotion(tmp_path: Path) -> None:
    ...
    hit_title = "A组织公开募捐风险提示"
    hit_snippet = "民政部门提醒基金会公开募捐合规风险，A组织需核对项目传播材料。"
    ...
    intent = GeneratedSearchIntent(
        ...
        project_module_id=None,
        content_kind="timely_intelligence",
        query="A组织 公开募捐 风险 通知",
        exclude_terms=[],
        source_inputs=["client:A组织"],
        reason="风险提示",
        priority=99,
        status="ready",
        input_hash="hash",
    ...
    with TestClient(app) as client:
        ...
        response = client.post(
            ...
            json={
                ...
                "scopeType": "client",
                "scopeId": "client_rici",
                "note": "不是A组织公开资料，不采纳泛营销网页",
            },
        )

        assert response.status_code == 200, response.text
        payload = response.json()
        assert any("不是A组织公开资料" in item for item in payload["excludeRules"])
        assert any("不是A组织公开资料" in item for item in payload["clarificationExamples"])
