...
def create_client_record(client: TestClient, name: str = "rollback-drill-p26") -> str:
...
def test_data_center_rollback_drill_p26(tmp_path: Path):
    ...
    assert dry_payload.get("wouldKeepEvidenceLabels") is True
    ...
    assert applied_payload.get("wouldKeepEvidenceLabels") is True
