...
注意:
- 这个测试调用火山云生产 cloud_backend(http://127.0.0.1)
...
def test_real_cloud_e2e_sync_then_profile(tmp_path: Path):
    ...
    assert profile["organization"]["name"] == "示例工作台"
    ...
    assert "管理员甲" in user_names, f"got: {user_names}"
    assert "乐乐" in user_names, f"got: {user_names}"
    assert "成员乙" in user_names, f"got: {user_names}"

    # 管理员甲应该是 CEO + isManager + dept=战略发展部
    guyuan = next(u for u in profile["users"] if u["fullName"] == "管理员甲")
