...
def test_profile_list_hides_promoted_candidates_and_legacy_short_excerpt_cards(tmp_path: Path) -> None:
    client = make_client(tmp_path)
    seed_client(client, client_id="client_a", name="示例工作台")
...
def test_profile_completion_candidate_filter_rejects_unrelated_foreign_results() -> None:
    ...
    relevant = CandidateHit(
        ...
        snippet="示例工作台项目报告与信息公开资料。",
        source="yiyu.love",
    )

    assert not _hit_matches_intelligence_context(noisy, intent, ["示例工作台"])
    assert _hit_matches_intelligence_context(relevant, intent, ["示例工作台"])
    ...
    assert not _hit_matches_intelligence_context(image_hit, intent, ["示例工作台"])
...
def test_verification_feedback_hides_candidate_and_saves_rule(tmp_path: Path) -> None:
    ...
    db.execute(
        ...
        VALUES('cand_rule', 'client', 'client_a', 'client_a', NULL, 'profile_completion',
            NULL, NULL, NULL, '示例工作台 图片结果', 'https://image.so.com/i?q=yiyu', 'https://image.so.com/i?q=yiyu',
    ...
    assert response.status_code == 200, response.text
    payload = response.json()
    assert "这不是示例工作台本身的信息" in payload["excludeRules"]
...
class _FakeProfileAi:
    ...
    def generate_general_fallback(self, *_args, **_kwargs):
        return AiStructuredResponse(
            content="广东省A组织公益基金会为广东省民政厅登记的慈善组织，公开资料可用于补充登记信息与业务范围。",
            judgment="资料属于广东省A组织公益基金会本身，可进入资料补全。",
...
def _profile_draft(scope: IntelligenceSearchScope) -> CandidateDraft:
    ...
    hit = CandidateHit(
        title="广东省A组织公益基金会 - 社会组织信息公开",
        url="https://gdngo.gdnpo.gov.cn/org/rici",
        snippet="广东省A组织公益基金会，登记机关广东省民政厅，慈善组织。",
    ...
    return CandidateDraft(
        ...
        matched_terms=["广东省A组织公益基金会", "登记信息", "信息公开"],
        confidence_score=92,
        signal_count=3,
    )
...
def test_profile_completion_promotion_requires_body_identity_mapping_and_ai(tmp_path: Path, monkeypatch) -> None:
    client = make_client(tmp_path)
    seed_client(client, client_id="client_rici", name="广东省A组织公益基金会")
    db = client.app.state.app_state.db
    scope = IntelligenceSearchScope(
        scope_type="client",
        scope_id="client_rici",
        client_id="client_rici",
        display_name="广东省A组织公益基金会",
    )
    ...
    monkeypatch.setattr(
        ...
        lambda _url: (
            "fetched",
            "广东省A组织公益基金会由广东省民政厅登记，统一社会信用代码为 53440000MJK850332A。业务范围包括慈善救助、公益项目资助、志愿服务项目、困难群体帮扶和公益传播。",
            "",
        ),
    )
    ...
    assert "广东省A组织公益基金会为广东省民政厅登记的慈善组织" in str(row["summary"] or "")
...
def test_profile_completion_does_not_promote_without_ai_summary(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _url: (
            "fetched",
            "广东省A组织公益基金会由广东省民政厅登记，业务范围包括慈善救助、公益项目资助、志愿服务项目和困难群体帮扶。",
            "",
        ),
    )
