...
def _scope(client_id: str = "client_rici", name: str = "广东省A组织公益基金会") -> IntelligenceSearchScope:
...
def _intent(
    ...
    *,
    ...
    return GeneratedSearchIntent(
        ...
        query=query,
        exclude_terms=[],
        source_inputs=["client:广东省A组织公益基金会"],
        reason="P5 防退化测试",
        priority=99,
        status="ready",
...
class _ReadyAi:
    ...
    def generate_general_fallback(self, *_args, **_kwargs):
        return AiStructuredResponse(
            content="广东省A组织公益基金会公开资料显示，该机构关注儿童心理健康与公益服务。",
            judgment="该内容与广东省A组织公益基金会当前客户画像相关。",
...
def test_p5_prototype_samples_and_example_domains_never_promote_or_enter_documents(tmp_path: Path, monkeypatch) -> None:
    ...
    scope = _scope()
    intent = _intent("广东省A组织公益基金会 儿童心理健康 项目资助征集开放", content_kind="profile_completion")
    ...
    monkeypatch.setattr(
        ...
        lambda _url: (
            "fetched",
            "广东省A组织公益基金会登记信息显示，该机构关注儿童心理健康、项目资助和公益服务，业务范围包含公益项目与社区服务。",
            "",
        ),
    )
...
def test_p5_noisy_counterexamples_remain_backend_rejections(tmp_path: Path, monkeypatch) -> None:
    ...
    _seed_client(db, client_id="client_rici", name="广东省A组织公益基金会")
    scope = _scope()
    intent = _intent("广东省A组织公益基金会 信息公开 年报", content_kind="profile_completion")
    _insert_intent(db, intent)
    monkeypatch.setattr(
        supply,
        "_fetch_page_text",
...
def test_p5_recruitment_directory_page_never_becomes_timely_candidate(tmp_path: Path) -> None:
    ...
    _seed_client(db, client_id="client_rici", name="广东省A组织公益基金会")
    scope = _scope()
    intent = _intent("广东省A组织公益基金会 公益创投 申报 征集", content_kind="timely_intelligence", intent_id="intent_jobui")
    _insert_intent(db, intent)

    def directory_fetcher(_query: str, source_config) -> list[CandidateHit]:
        return [
            CandidateHit(
                title="广东省A组织公益基金会 怎么样 - 职友集",
                url="https://www.jobui.com/company/14417762/",
                snippet="广东省A组织公益基金会工资待遇、招聘要求、公司评价。",
                source=source_config.source_name,
            )
        ]
...
def test_p5_profile_and_timely_streams_do_not_mix(tmp_path: Path, monkeypatch) -> None:
    ...
    _seed_client(db, client_id="client_rici", name="广东省A组织公益基金会")
    scope = _scope()
    timely_intent = _intent("广东省A组织公益基金会 社会组织 登记信息 年报", content_kind="timely_intelligence", intent_id="intent_static")
    profile_intent = _intent("广东省A组织公益基金会 公益创投 申报 征集", content_kind="profile_completion", intent_id="intent_opportunity")
    ...
    monkeypatch.setattr(
        ...
        lambda _url: (
            "fetched",
            "广东省A组织公益基金会近期公益创投申报征集通知，截止时间明确，需要判断是否符合申报资格。",
            "",
        ),
    )
...
def test_p5_user_visible_items_api_does_not_leak_internal_scores_or_candidates(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        _seed_client(db, client_id="client_rici", name="广东省A组织公益基金会")
        _seed_client(db, client_id="client_yiyu", name="示例工作台")
        db.execute(
            """
        ...
        db.execute(
            ...
            VALUES('item_other_client', 'timely_intelligence', 'client', 'client_yiyu', 'client_yiyu',
                '其他客户情报', '不应出现在A组织筛选结果中', ?, '其他分析', '其他影响', ?,
...
def test_p5_fetch_budget_limits_jobs_and_duplicates_merge(tmp_path: Path) -> None:
    ...
    scope = _scope()
    intents = [
        _intent("广东省A组织公益基金会 公益创投 申报 征集", content_kind="timely_intelligence", intent_id="intent_a"),
        _intent("广东省A组织公益基金会 政府购买服务 招标 采购", content_kind="timely_intelligence", intent_id="intent_b"),
        _intent("广东省A组织公益基金会 公开募捐 监管 风险", content_kind="timely_intelligence", intent_id="intent_c"),
    ]
