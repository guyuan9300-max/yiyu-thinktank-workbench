...
class FakeLocalAi:
    ...
    def generate_local_model_json(self, **kwargs: object) -> dict[str, object]:
        ...
        if "virtual_path" in prompt:
            return {
                "virtual_path": "日慈基金会/项目报告/云南调研/对外提交",
                "classification_tags": ["项目报告", "云南调研", "对外材料"],
                "recommended_owner": "益语智库",
        ...
        return {
            "title": "云南儿童资助项目报告",
            "purpose": "支撑日慈基金会理解项目进展、成果证据和后续协作重点。",
            "audience": "日慈基金会项目负责人、益语智库研究与交付团队。",
            ...
            "keywords": ["日慈基金会", "云南", "儿童资助", "项目报告"],
            "summary": "报告用于向日慈基金会呈现云南儿童资助项目的阶段性进展、主要发现和合作价值。",
...
def _seed_document(db: Database) -> tuple[str, str, str]:
    ...
    db.execute(
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        (client_id, "日慈基金会", "日慈", "公益", "foundation", "儿童资助项目合作方", "active", "#4F7DF3", now, now),
    )
    db.execute(
        """
        ...
        (
            ...
            "docx",
            "import",
            "报告面向日慈基金会，说明云南儿童资助项目进展、成果和后续建议。",
            "[]",
            now,
        ),
    )
    ...
    db.execute(
        ...
        (
            ...
            knowledge_document_id,
            1,
            "本报告提交给日慈基金会，说明云南儿童资助项目进展、成果证据、合作价值和后续风险。",
            "raw_hash",
            now,
        ),
    )
...
def test_processes_card_and_virtual_path_without_moving_original_file(tmp_path) -> None:
    ...
    assert card["purpose"].startswith("支撑日慈基金会")
    assert path_row is not None
    assert path_row["virtual_path"] == "日慈基金会/项目报告/云南调研/对外提交"
