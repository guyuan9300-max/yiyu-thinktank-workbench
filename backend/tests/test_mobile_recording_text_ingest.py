...
def seed_recording_context(client: TestClient) -> None:
    ...
    db.execute(
        ...
        VALUES('client_foundation', '日慈基金会', '', '', 'foundation', '', '', '#5B7BFE', ?, ?)
        """,
        (NOW, NOW),
    )
    db.execute(
    ...
    db.execute(
        ...
        VALUES('eline-q1', '日慈战略陪伴 Q1 复盘', 'custom', 'active', 'client_foundation', '日慈基金会', '[]', ?, ?)
        """,
        (NOW, NOW),
    )
    db.execute(
        ...
        VALUES(
            'task-local', '日慈项目复盘', '整理 Q1 项目复盘记录。', 'todo', 'todo', 'normal',
