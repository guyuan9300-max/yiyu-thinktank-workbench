...
def seed_cloud_session(client: TestClient, *, user_id: str = "user_admin_demo") -> None:
...
def seed_task_with_collaborator(
    ...
    *,
    ...
    db.execute(
        ...
        ) VALUES(?, ?, ?, ?, ?, 'normal', 'list-0', '管理员甲', '待确认', 'manual', '[]', '[]', ?, ?, '2026-04-27T09:00:00', '2026-04-27T09:00:00')
        """,
    ...
    db.execute(
        ...
        ) VALUES(?, 'org_1', 'user_admin_demo', '管理员甲', 'example_user@example.com', 0, 0, ?, '2026-04-27T09:00:00', '2026-04-27T09:00:00')
        """,
...
def test_local_task_records_hydrate_collaboration_state(tmp_path: Path):
    ...
    assert task["collaborators"][0]["userId"] == "user_admin_demo"
...
def test_cloud_backed_confirm_does_not_fake_success_when_cloud_fails(tmp_path: Path, monkeypatch):
    ...
    row = client.app.state.app_state.db.fetchone(
        ...
        """,
        ("task_cloud_confirm", "user_admin_demo"),
    )
...
def test_cloud_confirm_success_updates_local_inbox_state_even_when_shadow_upsert_skips(tmp_path: Path, monkeypatch):
    ...
    def cloud_accept(method: str, url: str, **kwargs):
        ...
        assert method == "POST"
        assert url.endswith("/api/v1/tasks/cloud_task_pending_local/collaborators/user_admin_demo/accept")
        return httpx.Response(
            200,
            json={
    ...
    row = client.app.state.app_state.db.fetchone(
        ...
        WHERE t.id = ? AND tc.user_id = ?
        """,
        ("task_cloud_pending_local", "user_admin_demo"),
    )
