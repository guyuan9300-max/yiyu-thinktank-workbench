...
def _db(tmp_path: Path) -> Database:
    ...
    db.execute(
        ...
        VALUES('client_a', 'A组织', 'A组织', '公益', '客户', '', '推进中', '2026-05-05T00:00:00', '2026-05-05T00:00:00')
        """
    )
    db.execute(
        """
    ...
    db.execute(
        ...
        VALUES('thread_a', 'client_a', 'A组织线程', '2026-05-05T00:00:00', '2026-05-05T00:00:00')
        """
    )
    db.execute(
        """
...
def test_thread_memory_records_projects_then_resolves_followup_chain(tmp_path: Path) -> None:
    ...
    first = fallback_update_thread_context(
        pack,
        prompt="请你帮我总结A组织的核心项目",
        answer_content="核心项目包括心灵魔法学院、心松松-教师心理关怀计划、青年支持计划、繁心计划。",
        retrieval_summary={},
...
def test_non_referential_new_question_does_not_apply_thread_memory(tmp_path: Path) -> None:
    ...
    refs = resolve_thread_references("你最喜欢管理员甲文章当中的哪一篇", pack)
...
def test_system_failure_does_not_update_thread_memory(tmp_path: Path) -> None:
    ...
    save_thread_context_pack(
        ...
        WorkspaceThreadContextPack(
            clientId="client_a",
            threadId="thread_a",
            mentionedObjects=["青年支持计划"],
            lastSelectedObject={"name": "青年支持计划"},
            turns=[],
        ),
        timestamp="2026-05-05T00:01:00",
    )
    ...
    assert pack.lastSelectedObject == {"name": "青年支持计划"}
...
def test_bootstrap_existing_thread_messages(tmp_path: Path) -> None:
    ...
    db.execute(
        ...
        VALUES(
            ...
            'A组织核心项目包括心灵魔法学院、心松松-教师心理关怀计划、青年支持计划、繁心计划。',
