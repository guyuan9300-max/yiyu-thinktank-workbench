...
def test_ingest_document_knowledge_moves_derived_intro_into_background_layer(tmp_path: Path):
    ...
    _insert_document_stub(
        ...
        client_id=client_id,
        document_id=document_id,
        file_name="B客户核心业务介绍.txt",
        excerpt="派生介绍稿",
    )
    source_path = tmp_path / "B客户核心业务介绍.txt"
...
def test_backfill_workspace_import_registers_existing_workspace_files(tmp_path: Path):
    ...
    (source_dir / "机构介绍.txt").write_text("A组织专注于儿童心理健康与教师支持。", encoding="utf-8")
...
def test_backfill_client_document_family_metadata_merges_timestamped_and_copy_variants(tmp_path: Path):
    ...
    titles = [
        "20260403_131702_# 一、B客户的核心定位与历史价值.docx",
        "20260403_130527_# 一、B客户的核心定位与历史价值.md",
        "副本B客户的核心定位与历史价值(1).pdf",
    ]
...
def test_openable_paths_recovers_stale_raw_path_by_same_file_name(tmp_path: Path):
    ...
    stale_path = root / "组织与战略" / "A组织战略合作 2_A组织_20260211.docx"
    actual_path = root / "_imports" / "imp_1" / "A组织战略合作 2_A组织_20260211.docx"
...
def test_openable_paths_marks_zero_byte_original_and_placeholder_markdown_invalid(tmp_path: Path):
    original_path = tmp_path / "client_workspace" / "A组织战略核心思想 2_A组织_20260211.pdf"
    markdown_path = tmp_path / "client_workspace" / "_v2_meta" / "markdown" / "A组织战略核心思想.md"
    original_path.parent.mkdir(parents=True)
    original_path.write_bytes(b"")
    markdown_path.parent.mkdir(parents=True)
    markdown_path.write_text("# A组织战略核心思想 2_A组织_20260211.pdf\n\n## 正文\n\n解析重试", encoding="utf-8")
...
def test_retrieve_knowledge_bundle_filters_placeholder_ready_documents(tmp_path: Path):
    ...
    _insert_ready_v2_document(
        ...
        file_name="A组织战略核心思想 2_A组织_20260211.pdf",
        visible_category="组织与战略",
        secondary_category="原始资料",
        preview_text="# A组织战略核心思想 2_A组织_20260211.pdf\n\n## 正文\n\n解析重试",
        doc_index_text="解析重试",
    ...
    _insert_ready_v2_document(
        ...
        file_name="A组织战略真实资料.pdf",
        visible_category="组织与战略",
        secondary_category="原始资料",
        preview_text="A组织战略真实资料，包含组织定位、战略变化和核心资产。",
        doc_index_text="战略 组织定位 核心资产",
        section_content="A组织战略真实资料说明组织定位、战略变化和核心资产，是有效可检索正文。",
    )

    bundle = retrieve_knowledge_bundle(db, tmp_path / "data", client_id, "A组织战略核心资产")
...
def test_retrieve_knowledge_bundle_intro_prefers_raw_files_in_pass1(tmp_path: Path):
    ...
    _insert_ready_v2_document(
        ...
        file_name="A组织机构介绍.docx",
        visible_category="机构介绍",
        secondary_category="原始资料",
        preview_text="机构介绍，聚焦儿童青少年心理健康。",
        doc_index_text="机构介绍 核心定位 心理健康",
        section_content="A组织是一家聚焦儿童青少年心理健康与心理教育的公益机构，机构定位清晰。",
    )
    _insert_ready_v2_document(
        db,
        client_id=client_id,
    ...
    bundle = retrieve_knowledge_bundle(db, tmp_path / "data", client_id, "介绍A组织的项目特点")
...
def test_extract_pdf_uses_ai_ocr_markdown_when_text_extract_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    class FakeAi:
        ...
        def generate_pdf_page_markdown(self, *, title: str, page_number: int, image_base64: str, mime_type: str) -> str:
            ...
            return f"## 页面 {page_number}\n\nB客户 战略价值与能力版图正文内容 {page_number}。" * 8
    ...
    assert "B客户 战略价值与能力版图正文内容 1" in text
    assert "B客户 战略价值与能力版图正文内容 2" in text
...
def test_extract_image_uses_visual_ocr_markdown(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    text, sections = extract_document(source, title="A组织战略.jpg", ai_service=ai)
...
def test_partial_ready_document_is_indexed_and_retrievable(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    _insert_document_stub(
        ...
        client_id=client_id,
        document_id=document_id,
        file_name="A组织战略超长扫描.pdf",
        excerpt="",
    )
    source = tmp_path / "A组织战略超长扫描.pdf"
    ...
    class FakeAi:
        def generate_pdf_page_markdown(self, *, title: str, page_number: int, image_base64: str, mime_type: str) -> str:
            return f"## 页面 {page_number}\n\nA组织战略资料第 {page_number} 页，说明儿童心理支持、项目升级和生态协作。" * 4
    ...
    bundle = retrieve_knowledge_bundle(db, tmp_path / "data", client_id, "A组织战略资料")
    assert any("A组织战略超长扫描" in item.get("title", "") for item in bundle.retrieval_summary["backgroundTrail"])
