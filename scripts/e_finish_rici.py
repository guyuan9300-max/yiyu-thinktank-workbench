#!/usr/bin/env python3
"""专跑A组织 card-gen 到完成(151篇)。全量已停, 无 claim 竞争。直接处理A组织 queued card-gen。"""
...
RC="client_demo_alpha"; db=Database(DD/"app.db")
def store(s):
    try:
...
try:
    ...
    print(f"AI ready={getattr(ai.get_health(),'ready','?')} | A组织起始 cards={cards()}/151")
    ...
    print(f"A组织待跑 card-gen: {len(tasks)} 篇")
    ...
    for i,t in enumerate(tasks):
        ...
        if (i+1)%5==0 or i==len(tasks)-1:
            print(f"  {i+1}/{len(tasks)} | A组织 cards={cards()}/151 | {round((time.time()-t0)/60,1)}min")
            prog({"done_idx":i+1,"total":len(tasks),"ok":ok,"fail":fail,"cards":cards(),"elapsed_min":round((time.time()-t0)/60,1),"running":True})
    el=time.time()-t0
    print(f">>> A组织 card-gen 完成 用时 {round(el/60,1)}min | 成功{ok} 失败{fail} | cards {cards()}/151")
